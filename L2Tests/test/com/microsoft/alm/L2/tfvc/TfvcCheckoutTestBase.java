...
public abstract class TfvcCheckoutTestBase extends L2Test {
  ...
  private void deleteWorkspaceIfExists(ServerContext context,
  ...
  protected void setUpCheckoutModel(VsoCheckoutPageModel model, String path) {
    // To avoid the test loading all the accounts for the user, we set the
    // account server we care about
  ...
  /**
   * Performs a checkout of the repository defined by the parameters from
   * environment, performs the test action, and cleans everything up afterwards.
   * @param action action that will receive the path to the checked out
   *     workspace.
   */
  protected void checkoutTestRepository(Consumer<java.nio.file.Path> action)
    ...
    VsoCheckoutPageModel model =
        (VsoCheckoutPageModel)checkoutModel.getVsoModel();
    ...
    for (int i = 0; i < table.getRowCount(); i++) {
      ...
      if (context.getTeamProjectReference().getName().equalsIgnoreCase(
              getTeamProject())) {
        index = i;
        break;
      }
    ...
    try {
      ...
      File tfvcFolder =
          new File(tempFolder, Path.combine(workspaceName, TFVC_FOLDER));
      File tfvcFolderWin =
          new File(tempFolder, Path.combine(workspaceName, TFVC_FOLDER_WIN));
      if (!model.isTfvcServerCheckout())
        Assert.assertTrue(tfvcFolder.exists() || tfvcFolderWin.exists());

      java.nio.file.Path workspacePath =
          Paths.get(tempFolder.getAbsolutePath(), workspaceName);
