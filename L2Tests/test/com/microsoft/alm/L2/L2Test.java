...
public abstract class L2Test extends UsefulTestCase {
  protected static final String ENV_UNIQUE_SUFFIX =
      "MSVSTS_INTELLIJ_UNIQUE_SUFFIX";
  static { Logger.setFactory(TestLoggerFactory.class); }
  ...
  public String getLegacyRepoUrl() { return legacyRepoUrl; }

  /**
   * Returns name unique enough for test purposes. Adds value of {@link
   * L2Test::ENV_UNIQUE_SUFFIX} to the passed prefix.
   */
  ...
  private AuthenticationInfo getAuthenticationInfo() {
    final AuthenticationInfo authenticationInfo =
        new AuthenticationInfo(user, pass, serverUrl, user);
    return authenticationInfo;
  }

  public void mockRepositoryContextForProject(final String serverUriString) {
    URI serverUri = URI.create(serverUriString);
    RepositoryContext mockContext = RepositoryContext.createGitContext(
        "/root/one", "repo1", "branch1", serverUri);
  ...
  private void loadContext() {
    ...
    final String message =
        "You must provide %s for the L2 tests through the following environment variable: %s (see Readme.md for more information)";
    Assert.assertFalse(
        String.format(message, "user", "MSVSTS_INTELLIJ_VSO_USER"),
        StringUtils.isEmpty(user));
    Assert.assertFalse(
        String.format(message, "pass", "MSVSTS_INTELLIJ_VSO_PASS"),
        StringUtils.isEmpty(pass));
    Assert.assertFalse(
        String.format(message, "serverUrl", "MSVSTS_INTELLIJ_VSO_SERVER_URL"),
        StringUtils.isEmpty(serverUrl));
    Assert.assertFalse(String.format(message, "teamProject",
                                     "MSVSTS_INTELLIJ_VSO_TEAM_PROJECT"),
                       StringUtils.isEmpty(teamProject));
    Assert.assertFalse(
        String.format(message, "repoUrl", "MSVSTS_INTELLIJ_VSO_GIT_REPO_URL"),
        StringUtils.isEmpty(repoUrl));
    Assert.assertFalse(String.format(message, "legacyRepoUrl",
                                     "MSVSTS_INTELLIJ_VSO_LEGACY_GIT_REPO_URL"),
                       StringUtils.isEmpty(legacyRepoUrl));
    Assert.assertFalse(
        String.format(message, "tfExe", "MSVSTS_INTELLIJ_TF_EXE"),
  ...
  protected void initializeTfEnvironment() {
    ...
    PluginServiceProvider.getInstance().getPropertyService().setProperty(
        PropertyService.PROP_TF_HOME, tfExe);
    ...
    MockTeamServicesSecrets secrets =
        (MockTeamServicesSecrets)TeamServicesSecrets.getInstance();
    secrets.forceUseAuthenticationInfo(info);
    secrets.setIgnoreWrites(true);
  }

  protected void mockTeamServicesSettingsService() {
    // create a context to use as the "saved context" but remove it like you're
    // starting from scratch
    final ServerContext context =
        ServerContextManager.getInstance().getUpdatedContext(getRepoUrl(),
                                                             false);
    ServerContextManager.getInstance().remove(context.getKey());
    assertTrue(
        ServerContextManager.getInstance().getAllServerContexts().isEmpty());

    // create the needed objects to restore a previous state
    final ServerContextState completeContextStateActual =
        new ServerContextState(context);
    ...
    final ServerContextState completeContextStateExtra =
        new ServerContextState(context);
    completeContextStateExtra.uri =
        context.getUri().toString().concat("-Extra");
    completeContextStateExtra.serverUri =
        context.getServerUri().toString().concat("-Extra");
    final SettingsState settingsState = new SettingsState();
    settingsState.serverContexts = new ServerContextState[] {
        completeContextStateActual, appContextState, completeContextStateExtra};
    final TeamServicesSettingsService settingsService =
        new TeamServicesSettingsService();
    settingsService.loadState(settingsState);

    // mimic restoring the state since it's already happened
    for (final ServerContext storedContext :
         settingsService.restoreServerContexts()) {
      ServerContextManager.getInstance().add(storedContext, false);
    }
    assertEquals(
        3, ServerContextManager.getInstance().getAllServerContexts().size());
  }

  /**
   * This method can be called by test methods to mock the JetBrains
   * SelectFilesDialog so that it returns the files passed in here.
   *
   * @param filesToSelect are the file you want the dialog to return for the
   *     test
   */
  protected void mockSelectFilesDialog(final List<File> filesToSelect) {
    ...
    MockFileSelector fileSelector =
        (MockFileSelector)IdeaFileSelector.getInstance();
  ...
  protected void setUp() throws Exception {
    ...
    try {
      myProjectFixture = IdeaTestFixtureFactory.getFixtureFactory()
                             .createFixtureBuilder(getTestName(true))
                             .getFixture();
      myProjectFixture.setUp();

      // Use the context info loaded earlier to setup the environment for TF
      // work
  ...
  public String getTestName(boolean lowercaseFirstLetter) {
    ...
    name = StringUtil.shortenTextWithEllipsis(name.trim().replace(" ", "_"), 12,
                                              6, "_");
  ...
  private void disposeRegisteredServerContexts() {
    ...
    for (final ServerContext context :
         ServerContextManager.getInstance().getAllServerContexts()) {
      ServerContextManager.getInstance().remove(context.getKey());
      context.dispose();
    }
  }
  ...
  private void disposeJerseyPool() throws InterruptedException {
    // Since there's no API to dispose Jersey thread pool, let's just give it a
    // bit more time to shutdown the pool.
    Thread.sleep(1000);
  }
  ...
  private void enableDebugLogging() {
    List<String> commonCategories = new ArrayList<String>(Arrays.asList(
        "#" + L2Test.class.getName(), "#" + GitHandler.class.getName(),
    ...
    // ArrayUtil.toStringArray(commonCategories));
    myTestStartedIndicator = createTestStartedIndicator();
    LOG.info(myTestStartedIndicator);
  }

  /**
   * Test methods can call this method to get the Server GitRepository object
   * given a project level server context and the repository name.
   *
  ...
  getServerGitRepository(final ServerContext projectLevelContext,
                         final String repositoryName) {
    final GitHttpClient gitClient = projectLevelContext.getGitHttpClient();
    final List<GitRepository> repos = gitClient.getRepositories(
        projectLevelContext.getTeamProjectReference().getName());
  ...
  /**
   * Test methods can call this method to remove an existing repository from the
   * server. This is usually done at the beginning of a test that will be
   ...
   */
  protected void
  removeServerGitRepository(final ServerContext projectLevelContext,
                            final String repositoryName) {
    final GitHttpClient gitClient = projectLevelContext.getGitHttpClient();
    final GitRepository foundRepo =
        getServerGitRepository(projectLevelContext, repositoryName);
    if (foundRepo != null) {
  ...
  // TODO: Replace with defaultRunBare(ThrowableRunnable<Throwable>) after
  // update to IDEA 2020.3
  @Override
