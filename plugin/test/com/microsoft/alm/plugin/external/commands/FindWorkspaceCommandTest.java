...
public class FindWorkspaceCommandTest extends AbstractCommandTest {
  ...
  public void testConstructor_localPath() {
    final FindWorkspaceCommand cmd =
        new FindWorkspaceCommand("/path/localfile.txt", null, true);
  }

  @Test(expected = IllegalArgumentException.class)
  public void testConstructor_nullLocalPath() {
    @SuppressWarnings("ConstantConditions")
    FindWorkspaceCommand cmd = new FindWorkspaceCommand(null, null, true);
  }

  @Test
  public void testConstructor_collectionWorkspace() {
    final FindWorkspaceCommand cmd =
        new FindWorkspaceCommand("collectionName", "workspaceName", null);
  }

  @Test(expected = IllegalArgumentException.class)
  public void testConstructor_nullCollection() {
    final FindWorkspaceCommand cmd =
        new FindWorkspaceCommand(null, "workspaceName", null);
  }

  @Test(expected = IllegalArgumentException.class)
  public void testConstructor_nullWorkspace() {
    final FindWorkspaceCommand cmd =
        new FindWorkspaceCommand("collectionName", null, null);
  }
  ...
  public void testGetArgumentBuilder_localPath() {
    ...
    Assert.assertEquals("workfold -noprompt -login:username,pw",
                        builder.toString());
  ...
  public void findWorkspaceCommandShouldOnlyBeCachedWithoutLocalPathDefined() {
    Assert.assertFalse(new FindWorkspaceCommand("/tmp", null, true)
                           .shouldPrepareCachedRunner());
    Assert.assertTrue(new FindWorkspaceCommand("collection", "workspace", null)
                          .shouldPrepareCachedRunner());
  }
  ...
  public void testGetArgumentBuilder_collectionWorkspace() {
    ...
    final ToolRunner.ArgumentBuilder builder = cmd.getArgumentBuilder();
    Assert.assertEquals(
        "workfold -noprompt -collection:collectionName -workspace:workspaceName",
        builder.toString());
  ...
  public void testGetArgumentBuilder_authInfo() {
    final AuthenticationInfo authInfo =
        new AuthenticationInfo("userName", "password", "serverUri", "user");
    final FindWorkspaceCommand cmd =
  ...
  public void testParseOutput_warningOutput() {
    ...
    final String output =
        ...
        + "$/project1: /path";
    Workspace workspace =
        Objects.requireNonNull(cmd.parseOutput(output, "").getDetailed());
    Assert.assertEquals("MyWorkspace", workspace.getName());
    Assert.assertEquals("http://server:8080/tfs/",
                        workspace.getServerDisplayName());
  ...
  public void testParseOutput_authServerError() {
    final FindWorkspaceCommand cmd =
        new FindWorkspaceCommand("/path/localfile.txt", null, false);
    cmd.parseOutput("", FindWorkspaceCommand.AUTH_ERROR_SERVER +
  ...
  public void testParseOutput_authFederatedError() {
    final FindWorkspaceCommand cmd =
        new FindWorkspaceCommand("/path/localfile.txt", null, false);
    cmd.parseOutput(
        "", "An error occurred: " + FindWorkspaceCommand.AUTH_ERROR_FEDERATED +
                ": http://serverName:8080/tfs");
  }
}
