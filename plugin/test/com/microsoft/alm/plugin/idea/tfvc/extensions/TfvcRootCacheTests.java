...
public class TfvcRootCacheTests {
  ...
  public void cacheKnowsAnyParentOfAnEmptySpaceIsNotARoot() {
    ...
    myCache.putNoMappingsFor(path);
    assertEquals(TfvcRootCache.CachedStatus.NO_ROOT,
                 myCache.get(path.getParent()));
  }
  ...
  public void cacheDoesntKnowChildOfEmptySpaceStatus() {
    Path path = Paths.get("/tmp");
    myCache.putNoMappingsFor(path);
    assertEquals(TfvcRootCache.CachedStatus.UNKNOWN,
                 myCache.get(path.resolve("child")));
  }
  ...
  public void cacheKnowsAnyChildAMappedRootIsUnderIt() {
    Path path = Paths.get("/tmp");
    myCache.putMappings(Collections.singletonList(createMapping(path)));
    assertEquals(TfvcRootCache.CachedStatus.UNDER_MAPPING_ROOT,
                 myCache.get(path.resolve("child")));
  }
  ...
  public void cacheDestroysTheOlderItemOnConflictingUpdate() {
    ...
    // Any other siblings of that path aren't under mapping now, since the cache
    // evicts any contradictory information:
    assertEquals(TfvcRootCache.CachedStatus.UNKNOWN, myCache.get(sibling));

    // If we suddenly again get the information that the /tmp is a root, then
    // the children are under it:
    myCache.putMappings(Collections.singletonList(createMapping(path)));
    assertEquals(TfvcRootCache.CachedStatus.UNDER_MAPPING_ROOT,
                 myCache.get(child));
    assertEquals(TfvcRootCache.CachedStatus.UNDER_MAPPING_ROOT,
