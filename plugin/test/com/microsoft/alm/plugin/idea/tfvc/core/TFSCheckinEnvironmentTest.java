...
@PrepareForTest({CommandUtils.class, ServiceManager.class,
                 VersionControlPath.class, TfsFileUtil.class,
...
public class TFSCheckinEnvironmentTest extends IdeaAbstractTest {
  ...
  @Mock TFSVcs mockTFSVcs;

  @Mock Project mockProject;

  @Mock ProgressManager mockProgressManager;

  @Mock ServerContext mockServerContext;

  @Mock VcsNotifier mockVcsNotifier;

  @Mock NullableFunction mockNullableFunction;
  ...
  public void setUp() {
    ...
    when(mockServerContext.getUri())
        .thenReturn(URI.create("http://organization.visualstudio.com"));
  ...
  public void testCommit_Happy() {
    setupCommit();
    when(CommandUtils.checkinFiles(mockServerContext,
                                   ImmutableList.of("/path/to/file1",
                                                    "/path/to/file2",
                                                    "/path/to/file3"),
                                   comment, null))
        .thenReturn("12345");

    List<VcsException> exceptions = tfsCheckinEnvironment.commit(
        changes, comment, mockNullableFunction, null);
  ...
  public void testCommit_Exception() {
    setupCommit();
    when(CommandUtils.checkinFiles(any(ServerContext.class), any(List.class),
  ...
  private void mockAddFiles(List<String> pathsToReturn) {
    when(CommandUtils.addFiles(eq(mockServerContext), anyList()))
        .then(invocation -> {
          List<String> filesToAddPaths =
              invocation.getArgumentAt(1, List.class);
          if (toCanonicalPaths(filesToAddPaths)
                  .equals(toCanonicalPaths(filePaths)))
  ...
  public void testScheduleUnversionedFilesForAddition_FailedAdd() {
    ...
    assertEquals(
        TfPluginBundle
            .message(TfPluginBundle.KEY_TFVC_ADD_ERROR, filePaths.get(1))
            .replace('\\', '/'),
