...
@PrepareForTest({CommandUtils.class, ServiceManager.class, TFSVcs.class,
                 PersistentFS.class, RenameUtil.class})
public class RenameFileDirectoryTest extends IdeaAbstractTest {
  ...
  private final String CURRENT_FILE_PATH =
      Path.combine(PARENT_PATH, "file.txt");
  private final String NEW_FILE_PATH = Path.combine(PARENT_PATH, NEW_FILE_NAME);

  @Mock private PsiFile mockPsiFile;

  @Mock private PsiDirectory mockPsiDirectory;

  @Mock private VirtualFile mockVirtualFile;

  @Mock private VirtualFile mockVirtualParent;

  @Mock private RefactoringElementListener mockListener;

  @Mock private TFSVcs mockTFSVcs;

  @Mock private Project mockProject;

  @Mock private ServerContext mockServerContext;

  @Mock private PersistentFS mockPersistentFS;

  @Mock private PendingChange mockPendingChange;

  @Before
  public void setUp() {
    MockitoAnnotations.initMocks(this);
    PowerMockito.mockStatic(CommandUtils.class, ServiceManager.class,
                            TFSVcs.class, PersistentFS.class, RenameUtil.class);
  ...
  public void testExecute_BadElement() {
    ...
    RenameFileDirectory.execute(mockmethod, NEW_FILE_NAME, usageInfos,
                                mockListener);
  }
  ...
  public void testExecute_RenameFileNoChanges() {
    ...
    when(CommandUtils.getStatusForFiles(
             any(Project.class), eq(mockServerContext),
             eq(ImmutableList.of(CURRENT_FILE_PATH))))
        .thenReturn(Collections.EMPTY_LIST);

    RenameFileDirectory.execute(mockPsiFile, NEW_FILE_NAME, usageInfos,
                                mockListener);

    verifyStatic(times(1));
    CommandUtils.renameFile(eq(mockServerContext), eq(CURRENT_FILE_PATH),
                            eq(NEW_FILE_PATH));
    ...
    RenameUtil.doRenameGenericNamedElement(
        any(PsiElement.class), any(String.class), any(UsageInfo[].class),
  ...
  public void testExecute_RenameDirectoryNoChanges() {
    ...
    when(CommandUtils.getStatusForFiles(any(Project.class),
                                        eq(mockServerContext),
                                        eq(ImmutableList.of(dirName))))
        .thenReturn(Collections.EMPTY_LIST);

    RenameFileDirectory.execute(mockPsiFile, NEW_DIRECTORY_NAME, usageInfos,
                                mockListener);

    verifyStatic(times(1));
    CommandUtils.renameFile(eq(mockServerContext), eq(dirName),
                            eq(Path.combine("/path/to/the", "newDirectory")));
    PersistentFS.getInstance().processEvents(any(List.class));
    verify(mockListener).elementRenamed(mockPsiFile);
  ...
  public void testExecute_RenameFileEditChanges() {
    when(mockPendingChange.getChangeTypes())
        .thenReturn(ImmutableList.of(ServerStatusType.EDIT));
    when(mockVirtualFile.getPath()).thenReturn(CURRENT_FILE_PATH);
    when(CommandUtils.getStatusForFiles(
             any(Project.class), eq(mockServerContext),
             eq(ImmutableList.of(CURRENT_FILE_PATH))))
  ...
  public void testExecute_RenameFileEditRenameChanges() {
    when(mockPendingChange.getChangeTypes())
        .thenReturn(
            ImmutableList.of(ServerStatusType.EDIT, ServerStatusType.RENAME));
    when(mockVirtualFile.getPath()).thenReturn(CURRENT_FILE_PATH);
    when(CommandUtils.getStatusForFiles(
             any(Project.class), eq(mockServerContext),
             eq(ImmutableList.of(CURRENT_FILE_PATH))))
  ...
  public void testExecute_RenameFileUnversionedChange() {
    when(mockPendingChange.getChangeTypes())
        .thenReturn(ImmutableList.of(ServerStatusType.ADD));
    when(mockVirtualFile.getPath()).thenReturn(CURRENT_FILE_PATH);
    when(CommandUtils.getStatusForFiles(
             any(Project.class), eq(mockServerContext),
             eq(ImmutableList.of(CURRENT_FILE_PATH))))
    ...
    verifyStatic(times(1));
    RenameUtil.doRenameGenericNamedElement(mockPsiFile, NEW_FILE_NAME,
                                           usageInfos, mockListener);
    verify(mockListener).elementRenamed(mockPsiFile);

    verifyStatic(never());
    CommandUtils.renameFile(any(ServerContext.class), any(String.class),
                            any(String.class));
    PersistentFS.getInstance().processEvents(any(List.class));
  }
}
