...
@PrepareForTest({OperationExecutor.class, ServerContextManager.class,
                 CommandUtils.class, VcsHelper.class, VcsNotifier.class})
public class WorkspaceModelTest extends IdeaAbstractTest {
  ...
  final List<Workspace.Mapping> mappings =
      Collections.singletonList(new Workspace.Mapping("$/", "/path/", false));
  ...
  public void mockObjects() {
    ...
    when(authenticatedContext.getTeamProjectReference())
        .thenReturn(new TeamProjectReference());

    serverContextManager = mock(ServerContextManager.class);
    when(serverContextManager.get(anyString()))
        .thenReturn(authenticatedContext);
    when(serverContextManager.createContextFromTfvcServerUrl(
             any(URI.class), anyString(), anyBoolean()))
        .thenReturn(authenticatedContext);
    when(serverContextManager.createContextFromTfvcServerUrl(
             any(URI.class), Matchers.eq(""), anyBoolean()))
        .thenReturn(null);
    ...
    final Workspace workspace =
        new Workspace(server, name, computer, owner, comment, mappings,
                      Workspace.Location.SERVER);
    PowerMockito.mockStatic(CommandUtils.class);
    when(CommandUtils.getWorkspace(any(ServerContext.class), anyString()))
        .thenReturn(workspace);
    when(
        CommandUtils.getWorkspace(any(ServerContext.class), any(Project.class)))
        .thenReturn(workspace);
    when(CommandUtils.getDetailedWorkspace(any(ServerContext.class),
                                           any(Project.class)))
        .thenReturn(workspace);
    when(CommandUtils.updateWorkspace(any(ServerContext.class),
                                      any(Workspace.class),
                                      any(Workspace.class)))
        .thenReturn("");
    when(CommandUtils.syncWorkspace(any(ServerContext.class), anyString()))
        .thenReturn(null);

    repositoryContext = RepositoryContext.createTfvcContext(
        "/path", name, "project1", workspace.getServerUri());
    repositoryContext_noProject = RepositoryContext.createTfvcContext(
        "/path", name, "", workspace.getServerUri());
    PowerMockito.mockStatic(VcsHelper.class);
    when(VcsHelper.getRepositoryContext(mockProject))
        .thenReturn(repositoryContext);
    when(VcsHelper.getRepositoryContext(mockProject2))
        .thenReturn(repositoryContext_noProject);

    mockVcsNotifier = mock(VcsNotifier.class);
    PowerMockito.mockStatic(VcsNotifier.class);
    when(VcsNotifier.getInstance(any(Project.class)))
        .thenReturn(mockVcsNotifier);
  }
  ...
  public void testLoading() {
    ...
    // Loading flag is special, we fire the event every time the setter is
    // called
    m.setLoading(true);
    Assert.assertEquals(true, m.isLoading());
    observer.assertAndClearLastUpdate(m, WorkspaceModel.PROP_LOADING);
  ...
  public void testValidate() {
    final WorkspaceModel m = new WorkspaceModel();
    Assert.assertEquals(TfPluginBundle.KEY_WORKSPACE_DIALOG_ERRORS_NAME_EMPTY,
                        m.validate().getValidationMessageKey());
    m.setName(name);
    Assert.assertEquals(
  ...
  public void testLoadWorkspace_withProject() {
    ...
    ArgumentCaptor<Runnable> taskArgument =
    ...
    // Check the values in model now to make sure the match the workspace we
    // created
    Assert.assertEquals(comment, m.getComment());
    Assert.assertEquals(computer, m.getComputer());
    Assert.assertEquals(name, m.getName());
  ...
  public void testLoadWorkspace_withNoRepoUrl() {
    final WorkspaceModel m = new WorkspaceModel();
    final Project localMockProject = mock(Project.class);
    final RepositoryContext mockRepositoryContext =
        mock(RepositoryContext.class);
    ...
    when(VcsHelper.getRepositoryContext(localMockProject))
        .thenReturn(mockRepositoryContext);
    m.loadWorkspace(localMockProject);
  ...
  public void testLoadWorkspace_withNoTeamProjectName() {
    ...
    when(mockRepositoryContext.getTeamProjectName())
  ...
  public void testLoadWorkspace_NoRepoContextFound() {
    final WorkspaceModel m = new WorkspaceModel();
    // mockProject2 will return a repository context that does not have a
    // project this will in turn call a null server context to be returned
    when(serverContextManager.createContextFromTfvcServerUrl(
             URI.create("http://bad.example.com"), "bad team name", true))
        .thenReturn(null);
    final Project localMockProject = mock(Project.class);
    final RepositoryContext mockRepositoryContext =
        mock(RepositoryContext.class);
  ...
  public void testSaveWorkspaceInternal() {
    ...
    final Workspace oldWorkspace =
        new Workspace(server, name + "old", computer, owner, comment, mappings);
    m.saveWorkspaceInternal(authenticatedContext, oldWorkspace, newWorkspace,
                            null, mockProject, "/path", true, null);
    verify(mockVcsNotifier)
        .notifyImportantInfo(anyString(), anyString(),
