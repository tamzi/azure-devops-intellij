...
/**
 * This is an abstract Operation class to use as a base class for other
 * operations.
 */
public abstract class Operation {
  ...
  public static class CredInputsImpl implements Inputs {
    ...
    public boolean getPromptForCreds() { return promptForCreds; }
  }
  ...
  /**
   * Use this method to get a properly authenticated context based on the
   * repositoryContext given.
   *
   * @param repositoryContext
   * @return
   */
  protected static ServerContext
  getServerContext(final RepositoryContext repositoryContext,
                   final boolean forcePrompt, final boolean allowPrompt,
                   final Logger logger) {
    final ServerContext serverContext;
    logger.info(
        String.format("getServerContext: url=%s", repositoryContext.getUrl()));
    ...
    // This should be done on a background thread so as not to block UI or hang
    // the IDE Get the context before doing the server calls to reduce
    // possibility of using an outdated context with expired credentials
    if (ServerContextManager.getInstance().get(repositoryContext.getUrl()) !=
            null &&
        forcePrompt) {
      logger.info("getServerContext: context found. updating auth info");
      // The context already exists, but the credentials may be out of date, so
      // we need to update the auth info
      ServerContextManager.getInstance().updateAuthenticationInfo(
    ...
    if (repositoryContext.getType() == RepositoryContext.Type.GIT) {
      ...
      if (allowPrompt) {
        final List<ServerContext> authenticatedContexts =
            new ArrayList<ServerContext>();
        final List<Future> authTasks = new ArrayList<Future>();
        // TODO: get rid of the calls that create more background tasks unless
        // they run in parallel
        try {
          authTasks.add(OperationExecutor.getInstance().submitOperationTask(
              new Runnable() {
                @Override
                public void run() {
                  // Get the authenticated context for the gitRemoteUrl
                  // This should be done on a background thread so as not to
                  // block UI or hang the IDE Get the context before doing the
                  // server calls to reduce possibility of using an outdated
                  // context with expired credentials
                  final ServerContext context =
                      ServerContextManager.getInstance().getUpdatedContext(
                          repositoryContext.getUrl(), false);
                  if (context != null) {
          ...
          logger.warn(
              "getServerContext: failed to get authenticated server context",
              t);
          return null;
        }

        if (authenticatedContexts == null ||
            authenticatedContexts.size() != 1) {
        ...
        serverContext =
            ServerContextManager.getInstance().createContextFromGitRemoteUrl(
      ...
      serverContext =
          ServerContextManager.getInstance().createContextFromTfvcServerUrl(
              ...
              repositoryContext.getTeamProjectName(), allowPrompt);
    }
