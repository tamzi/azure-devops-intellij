  ...
  public static void verify(TfvcCheckoutResult result) throws VcsException {
    if (!result.getErrorMessages().isEmpty()) {
      ...
      throw new VcsException(TfPluginBundle.message(
          TfPluginBundle.KEY_TFVC_CHECKOUT_FAILED, message));
    }

    if (!result.getNotFoundFiles().isEmpty()) {
      List<String> failedPaths = result.getNotFoundFiles()
                                     .stream()
                                     .map(TfsLocalPath::getPath)
                                     .collect(Collectors.toList());
      String failedPathList = String.join(", ", failedPaths);
      throw new VcsException(TfPluginBundle.message(
          TfPluginBundle.KEY_TFVC_CHECKOUT_FILES_FAILED, failedPathList));
    }
  }
}
