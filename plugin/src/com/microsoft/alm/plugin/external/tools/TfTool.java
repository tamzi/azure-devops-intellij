...
/**
 * This is a static class that provides helper methods for running the TF
 * command line.
 */
public class TfTool {
  ...
  private static final String[] TF_WINDOWS_PROGRAMS = {"tf.exe", "tf.bat",
                                                       "tf.cmd"};
  ...
  private static final String SYSTEM_DRIVE =
      SystemHelper.getEnvironmentVariable("SystemDrive");

  // standard unix paths to check (Git uses similar ones for both
  // Linux/Unix/Mac)
  private static final File[] UNIX_PATHS = {new File("/opt/local/opt"),
                                            new File("/opt"),
                                            new File("/sbin"),
                                            new File("/usr/share"),
                                            new File("/usr/sbin"),
  ...
  public static String getValidLocation() {
    ...
    final String[] filenames =
        Platform.isWindows() ? TF_WINDOWS_PROGRAMS : TF_OTHER_PROGRAMS;

    // Verify the path leads to a tf command and exists
    for (final String filename : filenames) {
      if (StringUtils.endsWith(location, filename) &&
  ...
  /**
   * This method returns the path to the TF command line program that was saved
  ...
  public static String getLocation() {
    ...
    return PluginServiceProvider.getInstance().getPropertyService().getProperty(
        PropertyService.PROP_TF_HOME);
  }

  /**
   * Determines the version of the TF command being used and throws if the
  ...
  /**
   * Use this method to check the TF exit code and throw an appropriate
   * exception.
   */
  ...
  public static String tryDetectTf() {
    final String[] exeNames =
        Platform.isWindows() ? TF_WINDOWS_PROGRAMS : TF_OTHER_PROGRAMS;
    final File[] filePaths =
        Platform.isWindows() ? PROGRAM_FILE_PATHS : UNIX_PATHS;
    return tryDetectTf(exeNames, filePaths);
  }

  protected static String tryDetectTf(final String[] exeNames,
                                      final File[] filePaths) {
    try {
      // try looking at env path
      String exe = checkTfPath(SystemHelper.getEnvironmentVariable(TF_HOME_ENV),
                               exeNames);
  ...
  private static String getExeFromPath(final String[] exeNames) {
    ...
    if (StringUtils.isNotEmpty(envPath)) {
      ...
      for (final String path : paths) {
        ...
        if (StringUtils.startsWith(filePath.getName(), TF_DIRECTORY_PREFIX) &&
  ...
  /**
   * Search thru given directories to find one of the given acceptable tf
   * commands
   *
   * @param paths
   * @param exeNames
   * @return
   */
  private static String searchDirectories(final File[] paths,
                                          final String[] exeNames) {
    for (final File path : paths) {
      if (path.exists()) {
        for (final File subDirectory : path.listFiles()) {
          if (StringUtils.startsWith(subDirectory.getName(),
                                     TF_DIRECTORY_PREFIX) &&
              subDirectory.isDirectory()) {
            final String verifiedPath =
                checkTfPath(subDirectory.getPath(), exeNames);
            if (verifiedPath != null) {
              return verifiedPath;
            }
          }
   ...
   * @return
   */
  private static String checkTfPath(final String exePath,
                                    final String[] exeNames) {
