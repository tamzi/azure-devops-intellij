...
public class VisualStudioTfvcCommands {
  ...
  private static final Logger ourLogger =
      Logger.getInstance(VisualStudioTfvcCommands.class);

  private static final Pattern VERSION_PATTERN =
      Pattern.compile("\\d+\\.\\d+\\.\\d+\\.\\d+");
  private static final Pattern WORKFOLD_REPORT_PATTERN =
      Pattern.compile("^(.*?): (.*)$");
  private static final Pattern WORKFOLD_WORKSPACE_NAME_AND_OWNER_PATTERN =
      Pattern.compile("^(.*?) \\((.*)\\)$");

  private static final String AUTHENTICATION_ERROR_PREFIX = "TF30063:";

  @NotNull
  public static CompletionStage<ToolVersion>
  getVersionAsync(@NotNull Project project, @NotNull Path clientPath) {
    String basePath = project.getBasePath();
    return executeClientAndProcessOutputAsync(
        clientPath, basePath == null ? null : Paths.get(basePath),
  ...
  /**
   * Returns partial workspace information (only workspace path, owner,
   * collection name, and mappings) from the Visual Studio TF client.
   *
   * May asynchronously throw {@link ToolAuthenticationException} in case of an
   * authentication error coming from the client.
   *
   ...
   */
  @NotNull
  public static CompletionStage<Workspace>
  getPartialWorkspaceAsync(@NotNull Path clientPath,
                           @NotNull Path workspacePath) {
    return executeClientAndProcessOutputAsync(
        clientPath, workspacePath, Collections.singletonList("workfold"),
        output -> {
          String workspaceName = null, workspaceUser = null,
                 collectionUrl = null;
          ...
          for (String line : stdout) {
            ...
            if (matcher.matches()) {
              if (workspaceName == null) {
                ...
                Matcher workspaceNameAndOwnerMatcher =
                    WORKFOLD_WORKSPACE_NAME_AND_OWNER_PATTERN.matcher(
          ...
          return workspaceDataAvailable
              ? new Workspace(collectionUrl, workspaceName, null, workspaceUser,
                              null, mappings)
              : null;
        });
  }
  ...
  private static <T> CompletionStage<T> executeClientAndProcessOutputAsync(
      @NotNull Path clientPath, @Nullable Path workingDirectory,
    ...
    try {
      ApplicationManager.getApplication().executeOnPooledThread(() -> {
        try {
          Couple<List<String>> output =
              VisualStudioTfvcClient.executeClientAndGetOutput(
                  clientPath, workingDirectory, arguments);
