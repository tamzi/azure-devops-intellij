...
public class VisualStudioTfvcClient {
  ...
  private static final Logger ourLogger =
      Logger.getInstance(VisualStudioTfvcClient.class);

  private static final String[] SUPPORTED_VS_VERSIONS =
      new String[] {"2019", "2017"};
  private static final String[] SUPPORTED_VS_EDITIONS =
      new String[] {"Enterprise", "Professional", "Community"};
  ...
  private static final ToolVersion MINIMAL_SUPPORTED_VERSION =
      new ToolVersion(15, 0, 0, "");
  private static final String MINIMAL_SUPPORTED_VERSION_NICKNAME =
      "Visual Studio 2017";

  @Nullable
  public static Path getOrDetectPath(PropertyService propertyService) {
    String path = propertyService.getProperty(
  ...
  /**
   * Tries to detect path to the latest available TFVC client from a set of
   * hardcoded locations.
   *
  ...
  public static Path detectClientPath() {
    if (!SystemInfo.isWindows)
      return null;
    String programFilesPath = SystemInfo.is32Bit
                                  ? System.getenv("ProgramFiles")
                                  : System.getenv("ProgramFiles(x86)");
    if (StringUtils.isEmpty(programFilesPath))
      programFilesPath = "C:\\Program Files (x86)";
    Path visualStudioPath =
        Paths.get(programFilesPath, "Microsoft Visual Studio");
    for (String vsVersion : SUPPORTED_VS_VERSIONS) {
      ...
      for (String vsEdition : SUPPORTED_VS_EDITIONS) {
        ...
        Path tfExePath =
            editionPath.resolve(TF_EXE_PATH_RELATIVE_TO_EDITION_ROOT);
        if (tfExePath.toFile().isFile())
          return tfExePath;
      }
    }
  ...
  @NotNull
  public static CompletionStage<Void>
  checkVersionAsync(@NotNull Project project,
                    @NotNull Path visualStudioClientPath) {
    return VisualStudioTfvcCommands
        .getVersionAsync(project, visualStudioClientPath)
        .thenAccept(version -> {
          if (version == null)
            throw new VisualStudioClientVersionException(
                ToolVersion.UNKNOWN, MINIMAL_SUPPORTED_VERSION,
                MINIMAL_SUPPORTED_VERSION_NICKNAME);
  ...
  private static List<String> readLines(InputStream stream, String label)
      throws IOException {
   ...
   */
  static Couple<List<String>>
  executeClientAndGetOutput(@NotNull Path clientPath,
                            @Nullable Path workingDirectory,
                            @NotNull List<String> arguments)
      throws IOException, InterruptedException {
    ourLogger.info("Executing VS client: " + clientPath +
                   ", args: " + StringUtils.join(arguments, ','));
    ...
    Process client =
        new ProcessBuilder()
            .command(command)
            .directory(workingDirectory == null ? null
