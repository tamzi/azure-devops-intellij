...
public class LockCommand extends Command<String> {
  public static final Logger logger =
      LoggerFactory.getLogger(LockCommand.class);
  ...
  public enum LockLevel {
    ...
    LockLevel(final String value) { this.value = value; }
    ...
    public static LockLevel fromString(final String level) {
      if (StringUtils.equalsIgnoreCase(StringUtils.remove(level, "-"),
                                       CHECKIN.toString())) {
        return CHECKIN;
      } else if (StringUtils.equalsIgnoreCase(StringUtils.remove(level, "-"),
                                              CHECKOUT.toString())) {
        return CHECKOUT;
      } else if (StringUtils.isEmpty(level) ||
                 StringUtils.equalsIgnoreCase(level, NONE.toString())) {
        return NONE;
      }
  ...
  private static final String CANNOT_UNLOCK_SUFFIX =
      "It is not currently locked in your workspace.";
   ...
   * Constructor
   *
   * @param context This is the server context used for collection and login
   *     info (can be null)
   */
  public LockCommand(final ServerContext context, final String workingFolder,
                     final LockLevel lockLevel, final boolean recursive,
   ...
   * Folder333:
   * Folder333
   * TF14090: Cannot unlock $/tfsTest_03/model_branch_06. It is not currently
   * locked in your workspace.
   */
  @Override
  public String parseOutput(final String stdout, final String stderr) {
    ...
    if (StringUtils.isNotEmpty(stderr)) {
      ...
      for (final String line : getLines(stderr)) {
        if (StringUtils.startsWith(line, CANNOT_UNLOCK_PREFIX) &&
            StringUtils.endsWith(line, CANNOT_UNLOCK_SUFFIX)) {
          // Found a warning that the file could not be unlocked because it is
          // not locked Ignoring this error
          continue;
        }
        errors.append(line);
        errors.append("\n");
  ...
  public int interpretReturnCode(int returnCode) {
    // We will treat Partial success (returnCode == 1) as success. Errors are
    // still reported but we need to show what went well also.
