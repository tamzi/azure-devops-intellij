...
/**
 * This command returns a fully populated workspace object but credentials are
 * always required <p> For server workspaces a server call must be made and a
...
public class GetDetailedWorkspaceCommand extends Command<Workspace> {
  ...
  /**
   * @param collectionDisplayName the collection display name, i.e. an URI with
   *     spaces represented by " " (literal
   *                              space) instead of "%20" (URL-encoded space).
   */
  public GetDetailedWorkspaceCommand(String collectionDisplayName,
                                     String workspace,
   ...
   * -collection:http://organization.visualstudio.com/ WorkspaceName
   * ===========================================================================================================================================================================================================
  ...
  public Workspace parseOutput(final String stdout, final String stderr) {
    ...
    // the output should be in this order but just in case it changes we check
    // the prefixes first
    int count = 0;
    while (count < output.length) {
      if (StringUtils.startsWith(output[count], WORKSPACE_PREFIX)) {
        workspace =
            StringUtils.removeStart(output[count], WORKSPACE_PREFIX).trim();
        ...
        computer =
            StringUtils.removeStart(output[count], COMPUTER_PREFIX).trim();
        ...
        collection =
            StringUtils.removeStart(output[count], COLLECTION_PREFIX).trim();
      } else if (StringUtils.startsWith(output[count], LOCATION_PREFIX)) {
        location =
            StringUtils.removeStart(output[count], LOCATION_PREFIX).trim();
    ...
    return new Workspace(collection, workspace, computer, owner, comment,
                         mappings, Workspace.Location.fromString(location));
  }
}