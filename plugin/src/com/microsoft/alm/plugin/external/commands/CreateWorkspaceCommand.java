 ...
 * <p/>
 * workspace /new [/noprompt] [/template:<value>] [/computer:<value>]
 * [/comment:<value>|@valuefile] [/collection:<url>] [/location:server|local]
...
public class CreateWorkspaceCommand extends Command<String> {
  private static final String WORKSPACE_EXISTS_ERROR =
      "An error occurred: The workspace %s;.* already exists on computer .*";
  ...
  /**
   * Creates a command that will checkout a project to a workspace of selected
   * kind when called.
   *
   * @param context       This is the server context used for collection and
   *     login info (can be null)
   * @param workspaceName This is the current name of the workspace to update
   * @param location      workspace location type: either {@link
   *     Workspace.Location#LOCAL} or
   *                      {@link Workspace.Location#SERVER}.
   */
  public CreateWorkspaceCommand(ServerContext context, String workspaceName,
                                String comment, Workspace.FileTime fileTime,
  ...
  /**
   * Creates a command that will checkout a project to a local workspace when
   * called.
   *
   * @param context       This is the server context used for collection and
   *     login info (can be null)
  ...
  public CreateWorkspaceCommand(ServerContext context, String workspaceName,
                                String comment, Workspace.FileTime fileTime,
                                Workspace.Permission permission) {
    this(context, workspaceName, comment, fileTime, permission,
  ...
  /**
   * There is no useful output from this command unless there is an error. This
   * method parses the error and throws if one exists.
   */
  @Override
  ...
  protected void throwIfError(final String stderr) {
    if (StringUtils.isNotEmpty(stderr)) {
      final Pattern pattern =
          Pattern.compile(String.format(WORKSPACE_EXISTS_ERROR, workspaceName));
