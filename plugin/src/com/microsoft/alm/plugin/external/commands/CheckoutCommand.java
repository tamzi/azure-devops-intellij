...
/**
 * Makes one or more local files writable and creates "edit" pending changes for
 * them in the current workspace.
 */
public class CheckoutCommand extends Command<TfvcCheckoutResult> {
   ...
   */
  public CheckoutCommand(@NotNull ServerContext context,
  ...
  private static void parseStdErr(String stderr,
                                  List<TfsLocalPath> notFoundFiles,
   ...
   * <p>
   * Example stderr:<br>
   * <pre>{@code The item C:\FullPath\dir\file.txt could not be found in your
   * workspace, or you do not have permission to access it.}</pre>
   */
  @Override
  public TfvcCheckoutResult parseOutput(String stdout, String stderr) {
    ...
    return new TfvcCheckoutResult(checkedOutFiles, notFoundFiles,
                                  errorMessages);
  }

  @Override
  protected boolean shouldThrowBadExitCode() {
    // This command parses all the errors and reports them as an object
    // returned; no need to check the exit code.
    return false;
  }
}