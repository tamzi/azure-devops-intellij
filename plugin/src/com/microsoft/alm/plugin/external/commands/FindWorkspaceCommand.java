...
/**
 * This command only returns a partial workspace object that allows you to get
 * the name and server. To get the entire workspace object you should call
 * GetWorkspace with the workspace name. (This is one of the only commands that
 * expects to be a strictly local operation - no server calls - and so does not
...
public class FindWorkspaceCommand extends Command<WorkspaceInformation> {
  protected static final Logger logger =
      LoggerFactory.getLogger(FindWorkspaceCommand.class);

  protected static final String AUTH_ERROR_SERVER =
      "An error occurred: Access denied connecting to TFS server";
  protected static final String AUTH_ERROR_FEDERATED =
      "Federated authentication to this server requires a username and password.";
  private static final String WORKSPACE_COULD_NOT_BE_DETERMINED =
   ...
   * @param authenticationInfo    authentication info (required to get detailed
   *     information on server workspaces)
   * @param allowBasicInformation whether to allow to retrieve just basic
   *     information (which is possible even for
   *                              server workspaces without credentials
   * available).
   */
  public FindWorkspaceCommand(@NotNull final String localPath,
  ...
  public ToolRunner.ArgumentBuilder getArgumentBuilder() {
    ...
    if (StringUtils.isNotEmpty(localPath)) {
      // To find the workspace we set the working directory to the localPath and
      // call workfold with no arguments NOTE Calling workfold with the
      // localPath forces it to refresh the workspace from the server. Calling
      // it with
    ...
    if (authInfo != null) {
      ...
      logger.info(
          "Using fake credentials for a FindWorkspaceCommand on the local workspace \"" +
          localPath + "\"");
  ...
  public boolean shouldPrepareCachedRunner() {
    // This command may be called on non-workspace location, so we shouldn't
    // generally prepare a runner for the new workspace in advance. So, we
    // should cache only "static" invocations with no working directory
    // override.
    return StringUtils.isEmpty(localPath);
  }

  /**
   * Checks stderr for fatal errors. Will throw a corresponding exception on a
   * fatal error, will return true on any nonfatal error. Will return false if
   * no errors were discovered.
   */
  private boolean checkErrors(String stderr) {
    if (StringUtils.startsWith(stderr, AUTH_ERROR_SERVER) ||
        StringUtils.contains(stderr, AUTH_ERROR_FEDERATED)) {
      logger.warn("Authentication exception hit when running 'tf workfold'");

      // Basic information could be extracted even in case of authentication
      // error, so we should ignore the error.
      if (allowBasicInformation) {
        return true;
      }

      throw new ToolAuthenticationException();
    } else if (StringUtils.contains(stderr,
                                    WORKSPACE_COULD_NOT_BE_DETERMINED)) {
   ...
   * https://organization.visualstudio.com/ (authenticating as Personal Access
   * Token)  <-- line is optional
   * =====================================================================================================================================================
   ...
   */
  @Override
  public WorkspaceInformation parseOutput(final String stdout,
                                          final String stderr) {
    ...
    for (int x = 0; x < lines.length; x++) {
      if (lines.length > x + 2 && lines[x].contains("==========")) {
        ...
        Workspace workspace =
            new Workspace(collectionURL, workspaceName, "", "", "", mappings);
  ...
  /**
   * This method parses a single line of output splitting on ": " and returning
   * the value after the colon Example:  "   Name: myWorkspace"  =>
   * "myWorkspace"
   */
  ...
  protected boolean shouldThrowBadExitCode() {
    return !allowBasicInformation; // only check exit code if we aren't allowed
                                   // to return just basic information
  }
}
