...
/**
 * This command calls Info which returns local and server information about an
 * item in the workspace. <p/> info [/recursive] [/version:<value>]
 * <itemSpec>...
 */
public class InfoCommand extends Command<List<ExtendedItemInfo>> {
  ...
  public InfoCommand(final ServerContext context,
                     final List<String> itemPaths) {
    this(context, null, itemPaths);
  }

  public InfoCommand(final ServerContext context, final String workingFolder,
                     final List<String> itemPaths) {
    super("info", context);
  ...
  public ToolRunner.ArgumentBuilder getArgumentBuilder() {
    ToolRunner.ArgumentBuilder builder =
        super.getArgumentBuilder().setWorkingDirectory(workingFolder);
   ...
   */
  @Override
  public List<ExtendedItemInfo> parseOutput(final String stdout,
                                            final String stderr) {
    ...
    for (final String line : output) {
      if (StringUtils.startsWithIgnoreCase(line, "local information:")) {
        ...
        if (colonPos > 0) {
          final String key =
              prefix + line.substring(0, colonPos).trim().toLowerCase();
          final String value = colonPos + 1 < line.length()
  ...
  private ExtendedItemInfo getItemInfo(Map<String, String> propertyMap) {
    return new ExtendedItemInfo(
        propertyMap.get("server path"), propertyMap.get("local path"),
        propertyMap.get("server changeset"), propertyMap.get("changeset"),
        propertyMap.get("change"), propertyMap.get("type"),
        propertyMap.get("server lock"), propertyMap.get("server lock owner"),
