...
/**
 * Base class for all TF commands. This class provides an argument builder for
 * the common arguments for all commmands: name of the command, no prompt,
 * collection url, and login info. If no context is provided the collection and
 * login parameters are simply left off (this should be fine if the command is a
 * local command). All commands must also provide an implementation of the
 * parseOutput method. This method takes the output readers for command and
 * converts them into an instance of T
 *
 * @param <T>
 */
public abstract class Command<T> {
  ...
  private static final Pattern CHANGESET_NUMBER_PATTERN =
      Pattern.compile("#(\\d+)");
  private static final Pattern INVALID_DOLLAR_PATH_PATTERN = Pattern.compile(
      "TF10122: The path '(.*?)' contains a '\\$' at the beginning of a path component. Remove the '\\$' and try again.");
  ...
  public interface Listener<T> {
    /**
     * This method is called to notify the owner of progress made by the command
     * process.
     *
     * @param output
     * @param outputType
     ...
     */
    void progress(final String output, final int outputType,
                  final int percentComplete);

    /**
     * This method is called to notify the owner that the command process has
     * completed. Check the error parameter first to see if anything went wrong.
     * If the error is set, the result parameter may be null.
     *
     * @param result
     * @param error
     */
  ...
  public Command(final String name, final ServerContext context,
                 final boolean useProxyIfAvailable, final boolean logStdOut) {
    this.name = name;
  ...
  public ToolRunner.ArgumentBuilder getArgumentBuilder() {
    final ToolRunner.ArgumentBuilder builder =
        new ToolRunner.ArgumentBuilder().add(name).addSwitch("noprompt");
    if (context != null && context.getCollectionURI() != null) {
      ...
      try {
        builder.addSwitch("collection",
                          URLDecoder.decode(collectionURI, "UTF-8"));
      } catch (UnsupportedEncodingException e) {
        logger.warn(
            "Error while decoding collection url. Using encoded url instead",
            e);
  ...
  /**
   * This method starts the command after hooking up the listener. It returns
   * immediately and calls the listener when the command process finishes.
   *
   * @param listener
   */
    ...
    final ToolRunner runner = ToolRunnerCache.getRunningToolRunner(
        ...
        getArgumentBuilder(), new ToolRunner.Listener() {
          ...
          public void completed(final int returnCode) {
            ...
            T result = null;
            try {
              // TODO there are some commands that write errors to stdout and
              // simply return a non-zero exit code (i.e. when a workspace is not
              // found by name)
              // TODO we may want to pass in the return code to the parse method
              // or something like that to allow the command to inspect this info
              // as well.
  ...
  /**
   * This method is provided to allow callers to run the command and wait on the
   * result. You should probably not call this method on the main thread. You
   * should also limit this to fast local commands.
   *
   * @return
   */
  public T runSynchronously() {
    ...
    try {
      ...
      logger.info(seconds + " sec - elapsed time for " +
                  this.getArgumentBuilder().toString());
      Debug.println("", seconds + " sec - elapsed time for " +
                            this.getArgumentBuilder().toString());
    }
  }
   ...
   * @return returnCode
   */
  public int interpretReturnCode(final int returnCode) { return returnCode; }

  /**
   * Determines whether a cached tool runner instance should be created after
   * execution of this command. This runner will be used when a new command is
   * issued against the same working directory/tool location. See
   * {@link ToolRunnerCache#getKey} for details on cache key calculation.
   * <p/>
   * Usually, it is a good idea to prepare a runner in advance, because there's
   * always a possibility that new commands will be issued against the same
   * working directory as defined by the current command. <p/> It should be
   * overridden in rare cases when it isn't necessary, e.g. when it is known
   * that command is often used against temporary/non-workspace locations.
   */
  public boolean shouldPrepareCachedRunner() { return true; }

  public boolean skipVersionCheck() { return false; }

  protected NodeList evaluateXPath(final String stdout,
    ...
    try {
      final Object result =
          xpath.evaluate(xpathQuery, xmlInput, XPathConstants.NODESET);
  ...
  protected String getXPathAttributeValue(final NamedNodeMap attributeMap,
  ...
  protected String[] getLines(final String buffer, final boolean skipWarnings) {
    final List<String> lines = new ArrayList<String>(
        Arrays.asList(buffer.replace("\r\n", "\n").split("\n")));
    if (skipWarnings) {
      while (lines.size() > 0 &&
             StringUtils.startsWithIgnoreCase(lines.get(0), WARNING_PREFIX)) {
        lines.remove(0);
      }
    }
  ...
  /**
   * This method is used by Checkin and CreateBranch to parse out the changeset
   * number.
   *
   * @param buffer
   * @return
  ...
  /**
   * This method evaluates a line of output to see if it contains something that
   * is expected. If not, it returns false. There are 3 kinds of expected lines:
   ...
   */
  protected boolean isOutputLineExpected(final String line,
                                         final String[] expectedPrefixes,
                                         final boolean filePathsAreExpected) {
    final String trimmed = line != null ? line.trim() : null;
    if (StringUtils.isNotEmpty(trimmed)) {
      // If we are expecting file paths, check for a file path pattern (ex.
      // /path/path2:)
  ...
  protected String getFilePath(final String path, final String filename,
  ...
  /**
   * This method parses a single line of output returning the mapping if one was
   * found Examples:
  ...
  protected Workspace.Mapping getMapping(final String line) {
    final boolean isCloaked =
        StringUtils.startsWithIgnoreCase(line.trim(), "(cloaked)");
  ...
  /**
   * If a bad exit code is detected then we should throw an exception. In some
   * instances though we want to throw a more specific message or we want the
   * IDE to handle the error differently so subclasses can override this
   *
   * @return
   */
  protected boolean shouldThrowBadExitCode() { return true; }
}
