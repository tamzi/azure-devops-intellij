...
public class TfvcEditFileProvider implements EditFileProvider {
  ...
  public TfvcEditFileProvider(TFSVcs vcs) { myVcs = vcs; }

  @Override
  public void editFiles(VirtualFile[] files) throws VcsException {
    List<Path> paths = Stream.of(files)
                           .map(file -> Paths.get(file.getPath()))
                           .collect(Collectors.toList());

    ServerContext serverContext = myVcs.getServerContext(true);
    TfvcCheckoutResult result =
        TfvcClient.getInstance(myVcs.getProject())
            .checkoutForEdit(serverContext, paths, false);
