...
public class LabelAction extends SimpleMultipleItemAction {
  public static final Logger logger =
      LoggerFactory.getLogger(LabelAction.class);
  ...
  execute(@NotNull final MultipleItemActionContext actionContext) {
    // Open the Apply Label dialog and allow the user to enter label name and
    // description
    final ApplyLabelDialog d =
        new ApplyLabelDialog(actionContext.project, actionContext.itemInfos);
    if (!d.showAndGet()) {
    ...
    runWithProgress(
        ...
        new Runnable() {
          public void run() {
            ...
            labels.addAll(CommandUtils.getLabels(actionContext.serverContext,
                                                 actionContext.workingFolder,
                                                 d.getLabelName()));
          }
        },
    ...
    if (!labels.isEmpty()) {
      logger.info("There is a label on the server already with the name " +
                  d.getLabelName());
      if (Messages.showOkCancelDialog(
              actionContext.project,
              TfPluginBundle.message(
                  TfPluginBundle.KEY_ACTIONS_TFVC_LABEL_OVERWRITE,
                  d.getLabelName()),
              TfPluginBundle.message(
                  TfPluginBundle.KEY_ACTIONS_TFVC_LABEL_TITLE),
              TfPluginBundle.message(
                  TfPluginBundle.KEY_ACTIONS_TFVC_LABEL_OVERWRITE_OK_TEXT),
              TfPluginBundle.message(
                  TfPluginBundle.KEY_ACTIONS_TFVC_LABEL_OVERWRITE_CANCEL_TEXT),
    ...
    runWithProgress(
        ...
        new Runnable() {
          public void run() {
            ...
            final boolean labelCreated = CommandUtils.createLabel(
                actionContext.serverContext, actionContext.workingFolder,
                d.getLabelName(), d.getLabelComment(), d.isRecursiveChecked(),
                d.getLabelItemSpecs());
            if (labelCreated) {
              successMessage.append(TfPluginBundle.message(
                  TfPluginBundle.KEY_ACTIONS_TFVC_LABEL_SUCCESS_CREATED,
                  d.getLabelName()));
            } else {
              successMessage.append(TfPluginBundle.message(
                  TfPluginBundle.KEY_ACTIONS_TFVC_LABEL_SUCCESS_UPDATED,
                  d.getLabelName()));
            }
          }
        },
        TfPluginBundle.message(
            TfPluginBundle.KEY_ACTIONS_TFVC_LABEL_PROGRESS_CREATING_LABEL));
