...
public class CheckoutAction extends SimpleMultipleItemAction {
  ...
  protected void execute(@NotNull MultipleItemActionContext actionContext) {
    ...
    ProgressManager.getInstance().run(new Task.Modal(
        project,
        TfPluginBundle.message(
            TfPluginBundle.KEY_ACTIONS_TFVC_CHECKOUT_PROGRESS),
        false) {
      @Override
      public void run(@NotNull ProgressIndicator indicator) {
        ...
        TfvcCheckoutResult checkoutResult = client.checkoutForEdit(
            actionContext.serverContext, filePaths, true);
        try {
          ...
          UIUtil.invokeLaterIfNeeded(
              ()
                  -> Messages.showErrorDialog(
                      TfPluginBundle.message(
                          TfPluginBundle.KEY_ACTIONS_TFVC_CHECKOUT_TITLE),
