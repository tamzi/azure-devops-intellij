...
public class LockItemsTableModel extends AbstractTableModel {
  ...
  public interface Listener extends EventListener { void selectionChanged(); }
  ...
  enum Column {
    ...
    Item(
        TfPluginBundle.message(TfPluginBundle.KEY_TFVC_LOCK_DIALOG_ITEM_COLUMN),
        550) {
    ...
    Lock(
        TfPluginBundle.message(TfPluginBundle.KEY_TFVC_LOCK_DIALOG_LOCK_COLUMN),
        110) {
      public String getValue(final ExtendedItemInfoWithSelection item) {
        final LockCommand.LockLevel level =
            LockCommand.LockLevel.fromString(item.info.getLock());
        switch (level) {
        case CHECKIN:
          return TfPluginBundle.message(
              TfPluginBundle.KEY_TFVC_LOCK_DIALOG_LOCK_LEVEL_CHECKIN);
        case CHECKOUT:
          return TfPluginBundle.message(
              TfPluginBundle.KEY_TFVC_LOCK_DIALOG_LOCK_LEVEL_CHECKOUT);
        }
        // "None" is just the empty string
        return StringUtils.EMPTY;
      }
    },
    LockOwner(TfPluginBundle.message(
                  TfPluginBundle.KEY_TFVC_LOCK_DIALOG_LOCKED_BY_COLUMN),
              130) {
      public String getValue(final ExtendedItemInfoWithSelection item) {
  ...
  private final EventDispatcher<Listener> myEventDispatcher =
      EventDispatcher.create(Listener.class);
  ...
  /**
   * This method looks at the first item in the list and selects all items that
   * are like it. I.e. if the first item is not locked then select all items
   * that are not locked OR if the first item IS locked, select all items that
   * are also locked. This assures that the one of the Lock or Unlock buttons
   * will be enabled.
   */
  private void setInitialSelection() {
    if (items != null && items.size() > 0) {
      final LockCommand.LockLevel firstItemLevel =
          LockCommand.LockLevel.fromString(items.get(0).info.getLock());
      final boolean selectIfNone = firstItemLevel == LockCommand.LockLevel.NONE;

      for (final ExtendedItemInfoWithSelection item : items) {
        final LockCommand.LockLevel currentLevel =
            LockCommand.LockLevel.fromString(item.info.getLock());
        if (currentLevel == LockCommand.LockLevel.NONE && selectIfNone) {
          item.selected = true;
        } else if (currentLevel != LockCommand.LockLevel.NONE &&
                   !selectIfNone) {
          item.selected = true;
        } else {
          item.selected = false;
