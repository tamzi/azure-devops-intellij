...
public class ProjectConfigurableForm {
  private static Logger ourLogger =
      Logger.getInstance(ProjectConfigurableForm.class);
  ...
  public ProjectConfigurableForm(final Project project) {
    ...
    downloadLink.setHyperlinkTarget(
        TfPluginBundle.message(TfPluginBundle.KEY_TFVC_SETTINGS_LINK_URL));
    downloadLink.setToolTipText(
        TfPluginBundle.message(TfPluginBundle.KEY_TFVC_SETTINGS_LINK_URL));
    pathLabel.setText(
        TfPluginBundle.message(TfPluginBundle.KEY_TFVC_SETTINGS_DESCRIPTION));
    tfExeField.addBrowseFolderListener(
        ...
        new FileChooserDescriptor(true, false, false, false, false, false)
            .withFileFilter(new Condition<VirtualFile>() {
              ...
              public boolean value(VirtualFile virtualFile) {
                return virtualFile.getName().equalsIgnoreCase(
                    SystemInfo.isWindows ? "tf.cmd" : "tf");
              }
            }));
      ...
      public void actionPerformed(ActionEvent actionEvent) {
        ...
        if (StringUtils.isEmpty(tfCmdPath)) {
          Messages.showErrorDialog(
              ...
              TfPluginBundle.message(
                  TfPluginBundle.KEY_TFVC_SETTINGS_PATH_EMPTY),
              TfPluginBundle.message(
                  TfPluginBundle.KEY_TFVC_TF_VERSION_WARNING_TITLE));
        ...
        if (!tfCmdFile.exists() || !tfCmdFile.isFile()) {
          Messages.showErrorDialog(
              myContentPane,
              TfPluginBundle.message(
                  TfPluginBundle.KEY_TFVC_SETTINGS_PATH_NOT_FOUND, tfCmdPath),
              TfPluginBundle.message(
                  TfPluginBundle.KEY_TFVC_TF_VERSION_WARNING_TITLE));
          downloadLinkPane.setVisible(true);
          return;
        }
        PropertyService.getInstance().setProperty(PropertyService.PROP_TF_HOME,
        ...
        try {
          ...
          Messages.showInfoMessage(
              myContentPane,
              TfPluginBundle.message(
                  TfPluginBundle.KEY_TFVC_SETTINGS_FOUND_EXE),
              TfPluginBundle.message(
                  TfPluginBundle.KEY_TFVC_TF_VERSION_WARNING_TITLE));
          downloadLinkPane.setVisible(false);
    ...
    useReactiveClientCheckBox.addActionListener(
        e -> refreshReactiveClientMemoryLimitAvailability());
    reactiveClientMemoryLimitLabel.setText(TfPluginBundle.message(
        TfPluginBundle.KEY_TFVC_SETTINGS_REACTIVE_CLIENT_MEMORY_LIMIT));
    ...
    if (SystemInfo.isWindows) {
      visualStudioClientLabel.setText(TfPluginBundle.message(
          TfPluginBundle.KEY_TFVC_SETTINGS_VISUAL_STUDIO_CLIENT));
      visualStudioClientField.addBrowseFolderListener(
          ...
          new FileChooserDescriptor(true, false, false, false, false, false)
              .withFileFilter(
                  virtualFile
                  -> virtualFile.getName().equalsIgnoreCase("tf.exe")));
      testVisualStudioClientButton.setText(TfPluginBundle.message(
          TfPluginBundle.KEY_TFVC_SETTINGS_VISUAL_STUDIO_CLIENT_TEST));
      testVisualStudioClientButton.addActionListener(
          action -> testVisualStudioClient());
    }
    ...
    //        if (Messages.showYesNoDialog(myProject, "Do you want to reset all
    //        stored passwords?", title, Messages.getQuestionIcon()) ==
    //        Messages.YES) {
    //          TFSConfigurationManager.getInstance().resetStoredPasswords();
    //          Messages.showInfoMessage(myProject, "Passwords reset
    //          successfully.", title);
    //        }
    //      }
    //    });
  ...
  public JComponent getContentPane() { return myContentPane; }
  ...
  private void refreshReactiveClientMemoryLimitAvailability() {
    String reactiveClientOptions =
        System.getenv(ReactiveTfvcClientHost.REACTIVE_CLIENT_OPTIONS_ENV);
    if (!Strings.isNullOrEmpty(reactiveClientOptions)) {
      reactiveClientMemoryLimitSpinner.setEnabled(false);
      reactiveClientMemoryLimitInfo.setText(TfPluginBundle.message(
    ...
    reactiveClientMemoryLimitSpinner.setEnabled(
        useReactiveClientCheckBox.isSelected());
  }
  ...
  public void load() {
    ...
    String tfLocation =
        propertyService.getProperty(PropertyService.PROP_TF_HOME);
    tfLocation =
        StringUtils.isEmpty(tfLocation) ? TfTool.tryDetectTf() : tfLocation;
    // if no path was found then default to an empty string
    originalTfLocation =
        StringUtils.isEmpty(tfLocation) ? StringUtils.EMPTY : tfLocation;
    if (StringUtils.isEmpty(originalTfLocation)) {
      downloadLinkPane.setVisible(true);
    }
    tfExeField.setText(originalTfLocation);

    boolean isReactiveClientEnabled =
        "true".equalsIgnoreCase(propertyService.getProperty(
            PropertyService.PROP_TFVC_USE_REACTIVE_CLIENT));
    useReactiveClientCheckBox.setSelected(isReactiveClientEnabled);

    if (SystemInfo.isWindows) {
      Path visualStudioTfLocation =
          VisualStudioTfvcClient.getOrDetectPath(propertyService);
      String vsClientPath = visualStudioTfLocation == null
                                ? null
                                : visualStudioTfLocation.toString();
    ...
    String reactiveClientMemory = propertyService.getProperty(
        PropertyService.PROP_REACTIVE_CLIENT_MEMORY);
    reactiveClientMemoryLimitSpinner.setValue(
        ReactiveTfvcClientHost.REACTIVE_CLIENT_DEFAULT_MEMORY_LIMIT);
  ...
  public void apply() {
    PropertyService propertyService = PropertyService.getInstance();
    propertyService.setProperty(PropertyService.PROP_TF_HOME,
                                getCurrentExecutablePath());

    boolean isReactiveClientEnabled = useReactiveClientCheckBox.isSelected();
    propertyService.setProperty(PropertyService.PROP_TFVC_USE_REACTIVE_CLIENT,
  ...
  public boolean isModified() {
    ...
    return !(
        propertyService.getProperty(PropertyService.PROP_TF_HOME)
            .equals(getCurrentExecutablePath()) &&
        isReactiveClientEnabled == useReactiveClientCheckBox.isSelected() &&
        Objects.equals(propertyService.getProperty(
                           PropertyService.PROP_VISUAL_STUDIO_TF_CLIENT_PATH),
                       getCurrentVisualStudioClientPath()) &&
        Objects.equals(propertyService.getProperty(
                           PropertyService.PROP_REACTIVE_CLIENT_MEMORY),
            ...
            -> ApplicationManager.getApplication().invokeLater(
                ()
                    -> Messages.showInfoMessage(
                        ...
                        TfPluginBundle.message(
                            TfPluginBundle
                                .KEY_TFVC_SETTINGS_FOUND_VS_CLIENT_EXE),
                        TfPluginBundle.message(
                            TfPluginBundle.KEY_TFVC_TF_VERSION_WARNING_TITLE)),
        ...
        .exceptionally(ex -> {
          ApplicationManager.getApplication().invokeLater(
              ()
                  -> Messages.showWarningDialog(
                      myContentPane,
                      LocalizationServiceImpl.getInstance().getExceptionMessage(
                          ex),
                      TfPluginBundle.message(
                          TfPluginBundle.KEY_TFVC_TF_VERSION_WARNING_TITLE)),
              ModalityState.stateForComponent(myContentPane));
  ...
  //    }
  //
  //    public void setSupportTfsCheckinPolicies(boolean
  //    supportTfsCheckinPolicies) {
  //        myTFSCheckBox.setSelected(supportTfsCheckinPolicies);
  //        updateNonInstalledCheckbox();
  //    }
  //
  //    public void setSupportStatefulCheckinPolicies(boolean
  //    supportStatefulCheckinPolicies) {
  //        myStatefulCheckBox.setSelected(supportStatefulCheckinPolicies);
  //        updateNonInstalledCheckbox();
  //    }
  //
  //    public void setReportNotInstalledCheckinPolicies(boolean
  //    reportNotInstalledCheckinPolicies) {
