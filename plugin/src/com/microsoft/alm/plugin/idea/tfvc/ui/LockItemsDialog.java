...
public class LockItemsDialog extends BaseDialogImpl {
  ...
  public LockItemsDialog(Project project, List<ExtendedItemInfo> items) {
    super(project,
          TfPluginBundle.message(TfPluginBundle.KEY_TFVC_LOCK_DIALOG_TITLE),
          null, TfPluginBundle.KEY_TFVC_LOCK_DIALOG_TITLE, true,
  ...
  protected JComponent createCenterPanel() {
    ...
    form.addListener(new LockItemsTableModel.Listener() {
      public void selectionChanged() { updateControls(); }
    });
  ...
  private static boolean canAllBeLocked(List<ExtendedItemInfo> items) {
    for (ExtendedItemInfo item : items) {
      final LockCommand.LockLevel level =
          LockCommand.LockLevel.fromString(item.getLock());
  ...
  public LockCommand.LockLevel getLockLevel() { return form.getLockLevel(); }

  public boolean getRecursive() { return form.getRecursive(); }

  private class LockAction extends AbstractAction {
    public LockAction() {
      super(TfPluginBundle.message(
          TfPluginBundle.KEY_TFVC_LOCK_DIALOG_LOCK_BUTTON));
  ...
  private class UnlockAction extends AbstractAction {
    public UnlockAction() {
      super(TfPluginBundle.message(
          TfPluginBundle.KEY_TFVC_LOCK_DIALOG_UNLOCK_BUTTON));
    }
