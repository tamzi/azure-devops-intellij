...
public class TfvcIntegrationEnabler extends VcsIntegrationEnabler {
  ...
  private static final Logger ourLogger =
      Logger.getInstance(TfvcIntegrationEnabler.class);
  ...
  public void enable(@NotNull Collection<? extends VcsRoot> vcsRoots) {
    // This override does the same as base method, but tries to determine a
    // workspace directory instead of using project.getBaseDir().
    Collection<VirtualFile> existingRoots =
        vcsRoots.stream()
            .filter(root -> {
              // TODO: Remove this suppression after migration to IDEA 2019.3.
              // AbstractVcs became non-generic in newer IDEA.
    ...
    ProgressManager.getInstance().run(
        new Task.Modal(myProject,
                       TfPluginBundle.message(
                           TfPluginBundle.KEY_TFVC_DETERMINE_WORKSPACE_TITLE),
                       true) {
          @Override
          public void run(@NotNull ProgressIndicator indicator) {
            Path workspacePath =
                determineWorkspaceDirectory(Paths.get(basePath), indicator);
  ...
  private void showVsAuthenticationErrorDialog(Path path) {
    ApplicationManager.getApplication().invokeLater(() -> {
      Messages.showWarningDialog(
          myProject,
          TfPluginBundle.message(
              TfPluginBundle.KEY_VISUAL_STUDIO_CLIENT_AUTHENTICATION_ERROR,
              path),
          TfPluginBundle.message(TfPluginBundle.KEY_VISUAL_STUDIO_CLIENT));
    }, ModalityState.NON_MODAL);
  }

  private CompletionStage<Workspace> getVsWorkspaceAsync(Path vsClient,
                                                         Path path) {
  ...
  @NotNull
  private Path
  determineWorkspaceDirectory(@NotNull Path projectBasePath,
                              @NotNull ProgressIndicator indicator) {
    Path vsClient =
        VisualStudioTfvcClient.getOrDetectPath(PropertyService.getInstance());
    ...
    do {
      ...
      try {
        ourLogger.info("Analyzing path \"" + path +
                       "\" using TF Everywhere client");
      ...
      if (workspace == null && vsClient != null) {
        ourLogger.info("Analyzing path \"" + path +
                       "\" using Visual Studio TFVC client (\"" + vsClient +
                       "\")");
        workspace =
            getVsWorkspaceAsync(vsClient, path).toCompletableFuture().join();
        if (workspace == null)
          ourLogger.info("Path \"" + path +
                         "\" has no Visual Studio TFVC workspace");
      }

      String currentPath = path.toAbsolutePath().toString();
      boolean correspondsToAnyMapping =
          workspace != null &&
          workspace.getMappings().stream().anyMatch(
              mapping
              -> FileUtil.pathsEqual(currentPath, mapping.getLocalPath()));
    ...
    ourLogger.warn("No workspace found, falling back to project base path \"" +
                   projectBasePath + "\"");
  ...
  private void addVcsRoot(@NotNull VirtualFile root) {
    ...
    List<VirtualFile> currentVcsRoots =
        Arrays.asList(vcsManager.getRootsUnderVcs(myVcs));

    List<VcsDirectoryMapping> mappings =
        new ArrayList<>(vcsManager.getDirectoryMappings(myVcs));
  ...
  private static void showNoVsClientDialog(@Nullable Project project) {
    ...
    Messages.showErrorDialog(
        project,
        TfPluginBundle.message(
            TfPluginBundle.KEY_TFVC_SETTINGS_VS_CLIENT_PATH_EMPTY),
  ...
  private static CompletionStage<AuthenticationInfo>
  getAuthenticationInfoAsync(@NotNull URI serverUri) {
    ...
    try {
      ...
      ServerContext serverContext =
          serverContextManager.get(serverUri.toString());
      if (serverContext != null) {
        ourLogger.info("Server context found for URL: " + serverUri);
        AuthenticationInfo authenticationInfo =
            serverContext.getAuthenticationInfo();
      ...
      AuthenticationProvider authenticationProvider =
          serverContextManager.getAuthenticationProvider(serverUri.toString());
      authenticationProvider.authenticateAsync(
          serverUri.toString(), new AuthenticationListener() {
            @Override
            ...
            @Override
            public void authenticated(AuthenticationInfo authenticationInfo,
                                      Throwable throwable) {
              ourLogger.info("Authentication result for URL " + serverUri +
  ...
  importWorkspaceAsync(@Nullable Project project,
                       @NotNull ProgressIndicator indicator,
    ...
    ourLogger.info("Checking if workspace under path \"" + workspacePath +
                   "\" is already imported");
    try {
      Workspace existingWorkspace =
          CommandUtils.getPartialWorkspace(workspacePath);
      // noinspection ConstantConditions // force null check just in case
      if (existingWorkspace != null) {
        ourLogger.info("Workspace under path \"" + workspacePath +
                       "\" is already imported, exiting");
        return CompletableFuture.completedFuture(true);
      }
    } catch (WorkspaceCouldNotBeDeterminedException ex) {
      ourLogger.info("No known workspace detected under path \"" +
                     workspacePath + "\"");
    }
    indicator.setFraction(1.0 / totalSteps);

    Path visualStudioClientPath =
        VisualStudioTfvcClient.getOrDetectPath(PropertyService.getInstance());
    ...
    ourLogger.info("Determining workspace information from client \"" +
                   visualStudioClientPath + "\" for path \"" + workspacePath +
                   "\"");
    return VisualStudioTfvcCommands
        .getPartialWorkspaceAsync(visualStudioClientPath, workspacePath)
        .thenCompose(vsWorkspace -> {
          if (vsWorkspace == null) {
            ourLogger.info("No workspace information, exiting");
            application.invokeLater(
                () -> showNoWorkspaceDetectedDialog(project));
            return CompletableFuture.completedFuture(false);
          }
          ...
          ourLogger.info("Gathering authentication info for URL: " +
                         collectionUri);
          return getAuthenticationInfoAsync(collectionUri)
              .thenApply(authenticationInfo -> {
                if (authenticationInfo == null) {
                ...
                ourLogger.info("Refreshing workspaces for server: " +
                               collectionUri);
                CommandUtils.refreshWorkspacesForServer(
                    authenticationInfo, collectionUri.toString());
                indicator.setFraction(4.0 / totalSteps);

                ourLogger.info(
                    "Checking if workspace was successfully imported from path: \"" +
                    workspacePath + "\"");
                Workspace workspace;
                try {
  ...
  protected boolean initOrNotifyError(@NotNull VirtualFile projectDir) {
    ...
    boolean success;
    try {
      success = ProgressManagerImpl.getInstance().run(
          new Task.WithResult<Boolean, Exception>(
              myProject,
              TfPluginBundle.message(
                  TfPluginBundle.KEY_TFVC_IMPORT_WORKSPACE_TITLE),
              true) {
            @Override
            protected Boolean compute(@NotNull ProgressIndicator indicator) {
              return importWorkspaceAsync(myProject, indicator,
                                          Paths.get(projectDir.getPath()))
                  .toCompletableFuture()
                  .join();
            }
          });
      ...
      if (success) {
        vcsNotifier.notifySuccess(TfPluginBundle.message(
