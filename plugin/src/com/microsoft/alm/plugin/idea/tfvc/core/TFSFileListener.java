...
public class TFSFileListener extends VcsVFSListener {
  public static final Logger logger =
      LoggerFactory.getLogger(TFSFileListener.class);

  public TFSFileListener(Project project, TFSVcs vcs) { super(project, vcs); }
  ...
  protected String getSingleFileAddPromptTemplate() {
    // pass {0} as a param because the current {0} in the string needs to be
    // replaced by something or else there is an error the {0} will be replaced
    // higher up by the file name that we don't have here
    return TfPluginBundle.message(TfPluginBundle.KEY_TFVC_ADD_PROMPT, "{0}");
  }

  @Override
  protected void
  executeAdd(@NotNull List<VirtualFile> addedFiles,
    ...
    if (UndoManager.getInstance(myProject).isUndoInProgress()) {
      logger.info(
          "{} files won't be added into TFS since Undo action is in progress",
          addedFiles.size());
      return;
    }
            ...
            -> ProgressManager.getInstance()
                   .runProcessWithProgressSynchronously(
                       ()
                         ...
                         pendingChanges.addAll(client.getStatusForFiles(
                             TFSVcs.getInstance(myProject).getServerContext(
                                 true),
                             filePaths));
                       },
                       TfPluginBundle.message(
                           TfPluginBundle.KEY_TFVC_ADD_SCHEDULING),
                       false, myProject));
  ...
  protected void executeDelete() {
    // overriding so we don't do any special logic here for delete since it's
    // already been taken care of
  }

  @Override
  protected void
  performAdding(@NotNull final Collection<VirtualFile> addedFiles,
                @NotNull final Map<VirtualFile, VirtualFile> copyFromMap) {
    if (addedFiles.isEmpty()) {
      logger.warn(
          "TFSFileListener was asked to add 0 files under VCS; skipping");
      return;
    }
    ...
    ProgressManager.getInstance().runProcessWithProgressSynchronously(
        ()
          ...
          List<VcsException> exceptions =
              checkinEnvironment.scheduleUnversionedFilesForAddition(
                  new ArrayList<>(addedFiles));
          if (exceptions != null)
            errors.addAll(exceptions);
        },
        TfPluginBundle.message(TfPluginBundle.KEY_TFVC_ADD_PROGRESS), false,
        myProject);

    if (!errors.isEmpty()) {
      AbstractVcsHelper.getInstance(myProject).showErrors(errors,
                                                          TFSVcs.TFVC_NAME);
    }
  }
  ...
  protected String getSingleFileDeleteTitle() { return null; }

  protected String getSingleFileDeletePromptTemplate() { return null; }
  ...
  protected boolean isDirectoryVersioningSupported() { return true; }

  private void removeInvalidTFVCAddedFiles(List<VirtualFile> addedFiles) {
    Map<VirtualFile, FilePath> addedFilePaths =
        addedFiles.stream().collect(Collectors.toMap(
            vf -> vf, vf -> new LocalFilePath(vf.getPath(), vf.isDirectory())));
    Set<FilePath> invalidPaths =
        TFVCUtil
            .collectInvalidTFVCPaths((TFSVcs)myVcs,
                                     addedFilePaths.values().stream())
            .collect(Collectors.toSet());

    addedFiles.removeIf(
        file -> invalidPaths.contains(addedFilePaths.get(file)));
  }
}
