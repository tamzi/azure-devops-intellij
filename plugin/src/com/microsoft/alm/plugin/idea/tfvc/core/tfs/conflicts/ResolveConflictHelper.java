...
public class ResolveConflictHelper {
  private static final Logger logger =
      LoggerFactory.getLogger(ResolveConflictHelper.class);

  @NotNull private final Project project;
  @Nullable private final UpdatedFiles updatedFiles;
  @NotNull private final List<String> updateRoots;
  @Nullable private final MergeResults mergeResults;
  ...
  /**
   * Determines which type of merge conflict is present (content or name) and
   * then gives the user the ability to resolve the issue
   *
   * @param conflict
   * @param model
   * @throws VcsException
   */
  public void acceptMerge(final @NotNull Conflict conflict,
                          final ResolveConflictsModel model)
      throws VcsException {
    logger.info(
        String.format("Merging changes for file %s", conflict.getLocalPath()));
    final File conflictPath = new File(conflict.getLocalPath());
    final ServerContext context =
        TFSVcs.getInstance(project).getServerContext(false);
    ...
    final FilePath localPath = VersionControlPath.getFilePath(
        conflict.getLocalPath(), conflictPath.isDirectory());
    ...
    // need to do this before any rename changes take place b/c it could
    // overwrite the contents of the file
    if (isContentConflict(conflict)) {
      logger.info("Content conflict have been found so getting file contents");
      contentTriplet = populateThreeWayDiffWithProgress(conflict, conflictPath,
                                                        localPath, context);
    }

    if (isNameConflict(conflict)) {
      logger.info(
          "Rename conflict have been found so getting new name from user");
    ...
    if (isNameConflict(conflict) && isContentConflict(conflict)) {
      ...
      processBothConflicts(conflict, context, model, conflictPath,
                           contentTriplet, nameMergerResolution);
  ...
  @VisibleForTesting
  protected void
  processBothConflicts(final Conflict conflict, final ServerContext context,
                       final ResolveConflictsModel model,
                       final File conflictPath,
                       @Nullable final ContentTriplet contentTriplet,
                       final NameMergerResolution nameMergerResolution)
      throws VcsException {
    // take their name
    if (nameMergerResolution != null &&
        nameMergerResolution.userChoseTheirs()) {
      logger.debug("Taking their name before processing content conflict");
      // resolve name conflict but do not update updatedFiles yet because
      // content conflict still needs resolved
      resolveConflictWithProgress(
          conflict.getLocalPath(),
          ResolveConflictsCommand.AutoResolveType.TakeTheirs, context, model,
          false, nameMergerResolution);
      processContentConflict(context, model, contentTriplet,
                             VersionControlPath.getFilePath(
                                 nameMergerResolution.getResolvedLocalPath(),
                                 conflictPath.isDirectory()),
                             nameMergerResolution);
    } else {
      // keep your name so just skip to content resolution which will do the
      // resolve for you (it always does KeepYours)
      logger.debug("Keeping your name so continue to content conflict");
      processContentConflict(
          context, model, contentTriplet,
          VersionControlPath.getFilePath(conflict.getLocalPath(),
                                         conflictPath.isDirectory()),
          nameMergerResolution);
    }
  }

  private NameMergerResolution
  getMergedNameFromUser(final RenameConflict renameConflict) {
    ...
    final String mergedServerPath =
        ConflictsEnvironment.getNameMerger().mergeName(
            myNameChoice, theirNameChoice, project);
    ...
    return new NameMergerResolution(theirNameChoice, myNameChoice,
                                    mergedServerPath);
  }
  ...
  @VisibleForTesting
  protected void
  processRenameConflict(final Conflict conflict, final ServerContext context,
                        final ResolveConflictsModel model,
                        final NameMergerResolution nameMergerResolution)
      throws VcsException {
    // take their name
    if (nameMergerResolution != null &&
      ...
      resolveConflictWithProgress(
          conflict.getLocalPath(), nameMergerResolution.getTheirNameChoice(),
          ResolveConflictsCommand.AutoResolveType.TakeTheirs, context, model,
          true, nameMergerResolution);
    } else {
      // keep your name
      logger.debug("Keeping your name");
      resolveConflictWithProgress(
          conflict.getLocalPath(),
          ResolveConflictsCommand.AutoResolveType.KeepYours, context, model,
          true, nameMergerResolution);
    }
  }
   ...
   */
  @VisibleForTesting
  protected void processContentConflict(
    ...
    if (contentTriplet != null) {
      ...
      if (vFile != null) {
        try {
          ...
          resolved = ConflictsEnvironment.getContentMerger().mergeContent(
              contentTriplet, project, vFile, null);
        } catch (IOException e) {
          throw new VcsException(e);
        }
      } else {
        throw new VcsException(TfPluginBundle.message(
            TfPluginBundle.KEY_TFVC_CONFLICT_MERGE_LOAD_FAILED,
  ...
  @VisibleForTesting
  protected void resolveConflictWithProgress(
      final String localPath,
      final ResolveConflictsCommand.AutoResolveType type,
      final ServerContext context, final ResolveConflictsModel model,
      final boolean updateFiles,
      final NameMergerResolution nameMergerResolution) throws VcsException {
    resolveConflictWithProgress(localPath, localPath, type, context, model,
                                updateFiles, nameMergerResolution);
  }

  @VisibleForTesting
  protected void resolveConflictWithProgress(
      final String localPath, final String updatedPath,
      final ResolveConflictsCommand.AutoResolveType type,
      final ServerContext context, final ResolveConflictsModel model,
      final boolean updateFiles,
      final NameMergerResolution nameMergerResolution) throws VcsException {
    final VcsRunnable resolveRunnable = new VcsRunnable() {
      public void run() throws VcsException {
        resolveConflict(localPath, updatedPath, type, context, model,
                        updateFiles, nameMergerResolution);
      }
    };
    VcsUtil.runVcsProcessWithProgress(
        resolveRunnable,
        TfPluginBundle.message(
            TfPluginBundle.KEY_TFVC_CONFLICT_RESOLVING_PROGRESS_BAR),
        false, project);

    // The following code is in this method instead of resolveConflict so that
    // it will be called from tests
    if (nameMergerResolution != null &&
        StringUtils.isEmpty(nameMergerResolution.getResolvedLocalPath())) {
      // If we didn't get back what we expected from resolving the conflict,
      // just default to the path we have
      nameMergerResolution.setResolvedLocalPath(
          nameMergerResolution.getUserSelection());
    }
  }

  /**
   * Resolve a conflict based on the given local path, update the updatedFiles
   * if needed with the updated local path (this is needed for renames where the
   * local path changes once a resolve takes place), then reload the conflicts
   *
   * @param localPath
   * @param updatedPath
   * @param type
  ...
  @VisibleForTesting
  protected void
  resolveConflict(final String localPath, final String updatedPath,
                  final ResolveConflictsCommand.AutoResolveType type,
                  final ServerContext context,
                  final ResolveConflictsModel model, final boolean updateFiles,
                  final NameMergerResolution nameMergerResolution)
      throws VcsException {
    IdeaHelper.setProgress(
        ProgressManager.getInstance().getProgressIndicator(), 0.1,
        TfPluginBundle.message(
            TfPluginBundle.KEY_TFVC_CONFLICT_RESOLVING_STATUS, localPath));

    try {
      final List<Conflict> resolvedConflicts =
          CommandUtils.resolveConflictsByPath(context, Arrays.asList(localPath),
                                              type);
      if (resolvedConflicts.size() == 1 && nameMergerResolution != null) {
        ...
        nameMergerResolution.setResolvedLocalPath(
            resolvedConflicts.get(0).getLocalPath());
      }

      if (updateFiles && updatedFiles != null) {
        // use local path from conflict to update list because results from
        // command gives server name even if you keep your own change
        updatedFiles.getGroupById(FileGroup.MERGED_ID)
            .add(updatedPath, TFSVcs.getKey(), null);
      }

      // update progress
      IdeaHelper.setProgress(
          ProgressManager.getInstance().getProgressIndicator(), 0.5,
          TfPluginBundle.message(
              TfPluginBundle.KEY_TFVC_CONFLICT_RESOLVING_REFRESH));
  ...
  @VisibleForTesting
  protected ContentTriplet populateThreeWayDiffWithProgress(
      final Conflict conflict, final File conflictPath,
      final FilePath localPath, final ServerContext context)
      throws VcsException {
    final ContentTriplet contentTriplet = new ContentTriplet();
    final VcsRunnable runnable = new VcsRunnable() {
      public void run() throws VcsException {
        populateThreeWayDiff(conflict, conflictPath, localPath, context,
                             contentTriplet);
      }
    };
    VcsUtil.runVcsProcessWithProgress(
        runnable,
        TfPluginBundle.message(TfPluginBundle.KEY_TFVC_CONFLICT_MERGE_LOADING),
        false, project);
  ...
  @VisibleForTesting
  protected void
  populateThreeWayDiff(final Conflict conflict, final File conflictPath,
                       final FilePath localPath, final ServerContext context,
    ...
    IdeaHelper.setProgress(
        ProgressManager.getInstance().getProgressIndicator(), 0.1,
        TfPluginBundle.message(
            TfPluginBundle.KEY_TFVC_CONFLICT_MERGE_ORIGINAL));
    ...
    try {
      ...
      if (conflictPath.isFile()) {
        ...
        List<PendingChange> pendingChange =
            TfvcClient.getInstance(project).getStatusForFiles(
                context, Collections.singletonList(conflictLocalPath));
        if (pendingChange.size() > 1) {
          logger.warn(
              "Count of local changes for file \"{}\" is greater than 1: {}",
              conflictLocalPath, pendingChange.size());
        }
        PendingChange originalChange =
            pendingChange.size() > 0 ? pendingChange.get(0) : null;
        if (isMergeConflict(conflict, null)) {
          final String workingFolder =
              localPath.isDirectory() ? localPath.getPath()
                                      : localPath.getParentPath().getPath();
          final MergeConflict mergeConflict = (MergeConflict)conflict;
          final String sourcePath =
              mergeConflict.getMapping().getFromServerItem();
          final String targetPath =
              mergeConflict.getMapping().getToServerItem();
          final VersionSpec baseVersion = CommandUtils.getBaseVersion(
              context, workingFolder, sourcePath, targetPath);

          original = TFSContentRevision
                         .createRenameRevision(
                             project, localPath,
                             SystemHelper.toInt(baseVersion.getValue(), 1),
                             originalChange.getDate(), sourcePath)
                         .getContent();
          serverChanges =
              TFSContentRevision
                  .createRenameRevision(project, localPath,
                                        getMergeFromVersion(mergeConflict),
                                        originalChange.getDate(), sourcePath)
                  .getContent();
          myLocalChanges =
              CurrentContentRevision.create(localPath).getContent();
        } else {
          // get original contents of file
          if (originalChange != null) {
            final int version = Integer.parseInt(originalChange.getVersion());
            if (isNameConflict(conflict)) {
              final FilePath renamePath = VersionControlPath.getFilePath(
                  conflictLocalPath, conflictPath.isDirectory());
              original = TFSContentRevision
                             .createRenameRevision(
                                 project, renamePath, version,
                                 originalChange.getDate(),
                                 ((RenameConflict)conflict).getOldPath())
                             .getContent();
            } else {
              original = TFSContentRevision
                             .create(project, localPath, version,
                                     originalChange.getDate())
                             .getContent();
            }
          } else {
          ...
          IdeaHelper.setProgress(
              ProgressManager.getInstance().getProgressIndicator(), 0.5,
              TfPluginBundle.message(
                  TfPluginBundle.KEY_TFVC_CONFLICT_MERGE_SERVER));
          ...
          if (isNameConflict(conflict)) {
            final ChangeSet serverChange =
                CommandUtils.getLastHistoryEntryForAnyUser(
                    context, ((RenameConflict)conflict).getServerPath());
            final FilePath renamePath = VersionControlPath.getFilePath(
                conflictLocalPath, conflictPath.isDirectory());
            serverChanges =
                TFSContentRevision
                    .createRenameRevision(
                        project, renamePath, serverChange.getIdAsInt(),
                        serverChange.getDate(),
                        ((RenameConflict)conflict).getServerPath())
                    .getContent();
          } else {
            final ChangeSet serverChange =
                CommandUtils.getLastHistoryEntryForAnyUser(context,
                                                           conflictLocalPath);
            serverChanges =
                TFSContentRevision
                    .create(project, localPath, serverChange.getIdAsInt(),
                            serverChange.getDate())
                    .getContent();
          }
        }
        ...
        contentTriplet.baseContent =
            original != null ? original : StringUtils.EMPTY;
        contentTriplet.localContent =
            myLocalChanges != null ? myLocalChanges : StringUtils.EMPTY;
        contentTriplet.serverContent =
            serverChanges != null ? serverChanges : StringUtils.EMPTY;
      }
    } catch (Exception e) {
      logger.error("Error loading contents for files");
      throw new VcsException(TfPluginBundle.message(
          TfPluginBundle.KEY_TFVC_CONFLICT_LOAD_FAILED,
          localPath.getPresentableUrl(), e.getMessage()));
    }
  }

  public void
  acceptChanges(final @NotNull String conflict,
                final ResolveConflictsCommand.AutoResolveType type) {
  ...
  public void acceptTheirs(final @NotNull String conflict) {
    if (updatedFiles != null) {
      ...
      updatedFiles.getGroupById(FileGroup.UPDATED_ID)
          .add(conflict, TFSVcs.getKey(), null);
    }
  }
  ...
  public void skip(final @NotNull String conflict) {
    if (updatedFiles != null) {
      ...
      updatedFiles.getGroupById(FileGroup.SKIPPED_ID)
          .add(conflict, TFSVcs.getKey(), null);
    }
  }
  ...
  public static boolean isNameConflict(final @NotNull Conflict conflict) {
    // Return true if this is a Rename conflict, Name and Content conflict, or a
    // Merge(with rename) conflict
  ...
  public static boolean isContentConflict(final @NotNull Conflict conflict) {
    // Return true if this is a content conflict, Name and Content conflict, or
    // a Merge(with edit) conflict
  ...
  isMergeConflict(final @NotNull Conflict conflict,
                  final @Nullable ServerStatusType matchingChangeType) {
    if (Conflict.ConflictType.MERGE.equals(conflict.getType())) {
      final MergeConflict mergeConflict = (MergeConflict)conflict;
      if (matchingChangeType != null) {
        return mergeConflict.getMapping().getChangeTypes().contains(
            matchingChangeType);
      }
  ...
  public static int getMergeFromVersion(final MergeConflict mergeConflict) {
    if (mergeConflict == null || mergeConflict.getMapping() == null ||
        ...
        mergeConflict.getMapping()
                .getFromServerItemVersion()
                .getEnd()
                .getType() != VersionSpec.Type.Changeset) {
      logger.warn(
          "The merge conflict does not contain the correct From Version Range (End).");
      throw new IllegalArgumentException("mergeConflict");
    }
    return SystemHelper.toInt(mergeConflict.getMapping()
                                  .getFromServerItemVersion()
                                  .getEnd()
                                  .getValue(),
                              0);
  ...
  /**
   * Resolve the conflicts based on auto resolve type and then refresh the table
   * model to update the list of conflicts
   *
   * @param conflicts
   * @param type
   */
  public void
  acceptChangeAsync(final List<Conflict> conflicts,
                    final ResolveConflictsCommand.AutoResolveType type,
                    final ResolveConflictsModel model) {
    logger.info(String.format("Accepting changes to %s for file %s",
                              type.name(),
                              Arrays.toString(conflicts.toArray())));
    final Task.Backgroundable loadConflictsTask = new Task.Backgroundable(
        project,
        TfPluginBundle.message(
            TfPluginBundle.KEY_TFVC_CONFLICT_RESOLVING_PROGRESS_BAR),
  ...
  @VisibleForTesting
  protected void
  acceptChange(final List<Conflict> conflicts,
               final ProgressIndicator progressIndicator, final Project project,
               final ResolveConflictsCommand.AutoResolveType type,
               final ResolveConflictsModel model) {
    for (final Conflict conflict : conflicts) {
      try {
        progressIndicator.setText(TfPluginBundle.message(
            TfPluginBundle.KEY_TFVC_CONFLICT_RESOLVING_STATUS,
            conflict.getLocalPath()));
        final List<Conflict> resolved = CommandUtils.resolveConflictsByConflict(
            TFSVcs.getInstance(project).getServerContext(false),
            Arrays.asList(conflict), type);

        // check if error is a rename so the correct file name is displayed in
        // the Update Info tab
        if (resolved != null && resolved.size() > 0) {
          if (conflict instanceof RenameConflict &&
              ResolveConflictsCommand.AutoResolveType.TakeTheirs.equals(type)) {
        ...
        logger.error("Error while handling merge resolution: " +
                     e.getMessage());
        model.addError(ModelValidationInfo.createWithMessage(
            TfPluginBundle.message(TfPluginBundle.KEY_TFVC_CONFLICT_MERGE_ERROR,
                                   conflict.getLocalPath(), e.getMessage())));
      }
    }
  ...
  public void findConflicts(final ResolveConflictsModel model)
    ...
    try {
      for (final String updatePath : updateRoots) {
        conflicts.addAll(CommandUtils.getConflicts(
            TFSVcs.getInstance(project).getServerContext(false), updatePath,
            mergeResults));
      }
    } catch (Exception e) {
    ...
    Collections.sort(conflicts, new Comparator<Conflict>() {
      ...
      public int compare(Conflict conflict1, Conflict conflict2) {
        return conflict1.getLocalPath().compareToIgnoreCase(
            conflict2.getLocalPath());
      }
    });
