...
public class WorkspaceModel extends AbstractModel {
  ...
  public Workspace.Location getLocation() { return location; }
  ...
  public ModelValidationInfo validate() {
    if (StringUtils.isEmpty(getName())) {
      return ModelValidationInfo.createWithResource(
          PROP_NAME, TfPluginBundle.KEY_WORKSPACE_DIALOG_ERRORS_NAME_EMPTY);
    }
    ...
    OperationExecutor.getInstance().submitOperationTask(() -> {
      try {
        ...
        final RepositoryContext repositoryContext =
            VcsHelper.getRepositoryContext(project);
        if (repositoryContext == null ||
            StringUtils.isEmpty(repositoryContext.getUrl()) ||
            StringUtils.isEmpty(repositoryContext.getTeamProjectName())) {
          logger.warn(
              "loadWorkspace: Could not determine repositoryContext for project");
          throw new RuntimeException(TfPluginBundle.message(
              TfPluginBundle.KEY_WORKSPACE_DIALOG_ERRORS_CONTEXT_FAILED));
        }
        ...
        currentServerContext =
            ServerContextManager.getInstance().createContextFromTfvcServerUrl(
                ...
                repositoryContext.getTeamProjectName(), true);
        if (currentServerContext == null) {
          logger.warn(
              "loadWorkspace: Could not get the context for the repository. User may have canceled.");
          throw new NotAuthorizedException(TfPluginBundle.message(
              TfPluginBundle.KEY_WORKSPACE_DIALOG_ERRORS_AUTH_FAILED,
              repositoryContext.getUrl()));
        }

        logger.info("loadWorkspace: getting workspace");
        loadWorkspaceInternal(
            CommandUtils.getDetailedWorkspace(currentServerContext, project));
      } finally {
  ...
  public void loadWorkspace(final RepositoryContext repositoryContext,
    ...
    OperationExecutor.getInstance().submitOperationTask(() -> {
      try {
        ...
        loadWorkspaceInternal(
            CommandUtils.getWorkspace(currentServerContext, workspaceName));
      } finally {
  ...
  public void loadWorkspace(final ServerContext serverContext,
  ...
  private void loadWorkspaceInternal(final Workspace workspace) {
    ...
    if (oldWorkspace != null) {
      ...
      location = workspace.getLocation() == Workspace.Location.UNKNOWN
  ...
  public void saveWorkspace(final Project project,
    ...
    final Workspace newWorkspace =
        new Workspace(server, name, computer, owner, comment, mappings);

    // Using IntelliJ's background framework here so the user can choose to wait
    // or continue working
    final Task.Backgroundable backgroundTask = new Task.Backgroundable(
        project,
        TfPluginBundle.message(
            TfPluginBundle.KEY_WORKSPACE_DIALOG_PROGRESS_TITLE),
        true, PerformInBackgroundOption.DEAF) {
      @Override
      public void run(@NotNull final ProgressIndicator indicator) {
        saveWorkspaceInternal(serverContext, oldWorkspace, newWorkspace,
                              indicator, project, workspaceRootPath, syncFiles,
                              onSuccess);
      }
    };
  ...
  @VisibleForTesting
  protected void saveWorkspaceInternal(
      final ServerContext serverContext, final Workspace oldWorkspace,
    ...
    try {
      IdeaHelper.setProgress(
          indicator, 0.10,
          TfPluginBundle.message(
              TfPluginBundle.KEY_WORKSPACE_DIALOG_SAVE_PROGRESS_UPDATING));
      ...
      if (syncFiles) {
        IdeaHelper.setProgress(
            indicator, 0.30,
            TfPluginBundle.message(
                TfPluginBundle.KEY_WORKSPACE_DIALOG_SAVE_PROGRESS_SYNCING));
      ...
      IdeaHelper.setProgress(
          indicator, 1.00,
          TfPluginBundle.message(
              TfPluginBundle.KEY_WORKSPACE_DIALOG_SAVE_PROGRESS_DONE),
          true);

      if (onSuccess != null) {
        // Trigger the onSuccess callback on the UI thread (it is up to the
        // success handler to notify the user)
        IdeaHelper.runOnUIThread(onSuccess, false,
                                 ModalityState.defaultModalityState());
      } else {
        // Notify the user of success and provide a link to sync the workspace
        // (It doesn't make sense to tell the user we are done here if there is
        // another thread still doing work)
        VcsNotifier.getInstance(project).notifyImportantInfo(
            TfPluginBundle.message(
                TfPluginBundle.KEY_WORKSPACE_DIALOG_NOTIFY_SUCCESS_TITLE),
            TfPluginBundle.message(
                TfPluginBundle.KEY_WORKSPACE_DIALOG_NOTIFY_SUCCESS_MESSAGE),
            new NotificationListener() {
              @Override
              public void hyperlinkUpdate(@NotNull final Notification n,
      ...
      // TODO on failure we could provide a link that reopened the dialog with
      // the values they tried to save
      VcsNotifier.getInstance(project).notifyError(
          TfPluginBundle.message(
              TfPluginBundle.KEY_WORKSPACE_DIALOG_NOTIFY_FAILURE_TITLE),
          LocalizationServiceImpl.getInstance().getExceptionMessage(t));
    }
  }

  public void syncWorkspaceAsync(final ServerContext context,
      ...
      public void run(@NotNull final ProgressIndicator indicator) {
        try {
          ...
          VcsNotifier.getInstance(project).notifySuccess(
              ...
              TfPluginBundle.message(
                  TfPluginBundle
                      .KEY_WORKSPACE_DIALOG_NOTIFY_SUCCESS_SYNC_MESSAGE));
        } catch (final Throwable t) {
          VcsNotifier.getInstance(project).notifyError(
              TfPluginBundle.message(
                  TfPluginBundle.KEY_WORKSPACE_DIALOG_NOTIFY_FAILURE_TITLE),
