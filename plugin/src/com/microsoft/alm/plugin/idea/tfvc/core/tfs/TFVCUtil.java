...
public class TFVCUtil {
  ...
  public static boolean isFileUnderTFVCMapping(@NotNull Project project,
                                               FilePath filePath) {
  ...
  /**
   * Will return only invalid paths for TFVC (i.e. paths containing dollar in
   * any component, or $tf / .tf service directory). <p> Performs a quick check
   * (without checking every VCS mapping) because we often need this in
   * performance-sensitive contexts.
   */
  public static Stream<FilePath>
  collectInvalidTFVCPaths(@NotNull TFSVcs vcs,
                          @NotNull Stream<FilePath> paths) {
    ProjectLevelVcsManager vcsManager =
        ProjectLevelVcsManager.getInstance(vcs.getProject());
    List<FilePath> mappings =
        vcsManager.getDirectoryMappings(vcs)
            .stream()
            .map(mapping -> new LocalFilePath(mapping.getDirectory(), true))
            .collect(Collectors.toList());
    return paths.filter(
        path
        -> isInServiceDirectory(path) ||
               mappings.stream().anyMatch(
                   mapping -> hasIllegalDollarInAnyComponent(mapping, path)));
  }

  /**
   * Will check invalid paths for TFVC (i.e. paths containing dollar in any
   * component, or $tf / .tf service directory). <p> Performs a quick check
   * (without checking every VCS mapping) because we often need this in
   * performance-sensitive contexts.
   */
  public static boolean isInvalidTFVCPath(@NotNull TFSVcs vcs,
    ...
    return vcsManager.getDirectoryMappings(vcs)
        .stream()
        .map(mapping -> new LocalFilePath(mapping.getDirectory(), true))
  ...
  /**
   * Some commands (e.g. `tf status` or `tf get`) won't work if any of the paths
   * passed don't belong to a workspace, so we need to filter only the items
   * belonging to a local workspace before passing arguments to these commands.
   */
  @NotNull
  public static List<String>
  filterValidTFVCPaths(@NotNull Project project,
                       @NotNull Collection<FilePath> paths) {
    List<FilePath> mappingPaths = getMappingsFromWorkspace(project);
    List<String> filteredPaths = new ArrayList<>();
    for (FilePath path : paths) {
      // if we get a change notification in the $tf folder, we need to just
      // ignore it
      if (isInServiceDirectory(path)) {
        continue;
      }
      ...
      for (FilePath mappingPath : mappingPaths) {
        if (path.isUnder(mappingPath, false)) {
          // Ignore any paths that has '$' in any component under the mapping
          // root.
  ...
  private static List<FilePath>
  getMappingsFromWorkspace(@NotNull Project project) {
   ...
   */
  private static boolean hasIllegalDollarInAnyComponent(FilePath mapping,
                                                        FilePath localPath) {
    String relativePath =
        FileUtil.getRelativePath(mapping.getIOFile(), localPath.getIOFile());
