...
/**
 * Listener that intercepts file system actions and executes the appropriate
 * TFVC command if needed
 */
public class TFSFileSystemListener
    implements LocalFileOperationsHandler, Disposable {
  private static final Logger ourLogger =
      LoggerFactory.getLogger(TFSFileSystemListener.class);
  ...
  @Override
  public void dispose() {
    LocalFileSystem.getInstance().unregisterAuxiliaryFileOperationsHandler(
        this);
  }
  ...
  public boolean delete(@NotNull final VirtualFile virtualFile)
    ...
    if (TFVCUtil.isInvalidTFVCPath(
            vcs, new LocalFilePath(virtualFile.getPath(),
                                   virtualFile.isDirectory()))) {
      ourLogger.warn(
          "Invalid file name for TFVC, so not performing TFVC delete: {}",
          virtualFile.getPath());
      return false;
    }
    ...
    final VcsShowConfirmationOption.Value value =
        vcs.getDeleteConfirmation().getValue();
    ...
    // TODO: Currently, we cannot allow the users to undo directory deletion,
    // because there's no non-recursive `tf undo` command. In future, we may
    // allow this after we drop legacy client support and add a undo command
    // that won't touch the disk at all.
    //
    // Currently, `tf undo` will always try to restore the whole directory (with
    // ALL the files) which breaks the undo behavior.
    if (virtualFile.isDirectory()) {
      ourLogger.info(
          "Marking operation as non-undoable since directory \"{}\" is deleted",
          virtualFile);
      DocumentReference ref =
          DocumentReferenceManager.getInstance().create(virtualFile);
      UndoManager.getInstance(currentProject)
          .nonundoableActionPerformed(ref, false);
    }
    ...
    List<PendingChange> pendingChanges = tfvcClient.getStatusForFiles(
        serverContext, Collections.singletonList(virtualFile.getPath()));
    ...
    if (pendingChanges.isEmpty()) {
      ...
      ourLogger.trace("Delete command finished in " + time / 1_000_000_000.0 +
                      "s");

      // There's a possibility that the path in question was explicitly ignored
      // using .tfignore. In such case, TFVC client will report that the file
      // wasn't found and won't do anything with it. If this happened, then
    ...
    // start with assuming you don't need to revert but look at the pending
    // changes to see if that's incorrect
    ...
    for (final PendingChange pendingChange : pendingChanges) {
      StatusProvider.visitByStatus(new StatusVisitor() {
        public void scheduledForAddition(
            final @NotNull FilePath localPath, final boolean localItemExists,
        ...
        public void unversioned(final @NotNull FilePath localPath,
                                ...
                                final @NotNull ServerStatus serverStatus) {
          // only do something if it's an unversioned delete, the IDE will take
          // care of it otherwise
          if (pendingChange.getChangeTypes().contains(
                  ServerStatusType.DELETE)) {
            revert.set(true);
            success.set(true);
          }
        }
        ...
        @Override
        public void locked(@NotNull FilePath localPath, boolean localItemExists,
                           @NotNull ServerStatus serverStatus) {
          // nothing to do if it's locked
          success.set(false);
        }

        public void renamed(final @NotNull FilePath localPath,
                            final boolean localItemExists,
                            final @NotNull ServerStatus serverStatus) {
          // revert it and then delete it
          revert.set(true);
    ...
    if (revert.get()) {
      ...
      tfvcClient.undoLocalChanges(serverContext,
    ...
    if (success.get() && !isUndelete.get()) {
      ...
      // PendingChanges will always have at least 1 element or else we wouldn't
      // have gotten this far
      PendingChange pendingChange = pendingChanges.get(0);
      TfsPath itemToDelete =
          StringUtils.isNotEmpty(pendingChange.getSourceItem())
              ? new TfsServerPath(pendingChange.getWorkspace(),
                                  pendingChange.getSourceItem())
              : TfsFileUtil.createLocalPath(pendingChange.getLocalItem());
      TfvcDeleteResult operationResult = tfvcClient.deleteFilesRecursively(
          serverContext, Collections.singletonList(itemToDelete));
      operationResult.throwIfErrorMessagesAreNotEmpty();
  ...
  @Override
  public boolean move(final VirtualFile virtualFile,
                      final VirtualFile toDirectory) throws IOException {
    ourLogger.info(String.format("Moving file %s to %s", virtualFile.getPath(),
                                 toDirectory.getPath()));
    return renameOrMove(virtualFile, Path.combine(toDirectory.getPath(),
                                                  virtualFile.getName()));
  }

  @Nullable
  @Override
  public File copy(final VirtualFile virtualFile,
                   final VirtualFile virtualFile1, final String s) {
    return null;
  }

  @Override
  public boolean rename(final VirtualFile virtualFile, final String s)
      throws IOException {
    ourLogger.info(
        String.format("Renaming file %s to %s", virtualFile.getName(), s));
    return renameOrMove(virtualFile,
                        Path.combine(virtualFile.getParent().getPath(), s));
  }

  /**
   * The point of this method is to perform undo operation in TFS to restore the
   * file on disk in case we're in middle of the IDE undo action.
   *
   * @param dir  the directory in which the file is being created.
   * @param name the name of the new file.
   * @return true if the handler has performed the file creation, false if the
   *     creation needs to be performed through
   * standard core logic.
   */
  @Override
  public boolean createFile(@NotNull final VirtualFile dir,
    ...
    List<PendingChange> changes = client.getStatusForFiles(
        serverContext, Collections.singletonList(path));
    ...
    for (PendingChange change : changes) {
      StatusProvider.visitByStatus(new StatusProvider.StatusAdapter() {
        @Override
        public void scheduledForDeletion(@NotNull FilePath localPath,
    ...
    if (performUndo.get()) {
      ...
      List<TfsLocalPath> undone = client.undoLocalChanges(
          serverContext, Collections.singletonList(filePath));
  ...
  @Override
  public boolean createDirectory(@NotNull final VirtualFile dir,
                                 @NotNull final String name) {
    return false;
  }
  ...
  @Override
  public void
  afterDone(final ThrowableConsumer<LocalFileOperationsHandler, IOException>
                throwableConsumer) {
    // nothing to do
  }
   ...
   */
  private boolean renameOrMove(final VirtualFile oldFile, final String newPath)
      throws IOException {
    ...
    LocalFilePath oldFilePath =
        new LocalFilePath(oldFile.getPath(), isDirectory);
    if (TFVCUtil.isInvalidTFVCPath(vcs, oldFilePath)) {
      ourLogger.warn("Invalid old TFVC path, ignore rename or move: {}",
                     oldFilePath);
      return false;
    }
    ...
    if (TFVCUtil.isInvalidTFVCPath(vcs, newFilePath)) {
      ourLogger.warn("Invalid new TFVC path, ignore rename or move: {}",
                     newFilePath);
      return false;
    }
    ...
    try {
      ...
      List<PendingChange> pendingChanges = client.getStatusForFiles(
          vcs.getServerContext(true), ImmutableList.of(oldPath));

      // ** Rename logic **
      // If 1 change and it's a candidate add that means it's a new unversioned
      // file so rename through the file system Anything else can be renamed
      // Deleted files should not be at this point since IDE disables rename
      // option for them
      if (pendingChanges.size() == 1 && pendingChanges.get(0).isCandidate() &&
        ...
        if (!client.renameFile(vcs.getServerContext(true), Paths.get(oldPath),
                               Paths.get(newPath))) {
          throw new IOException("Couldn't rename file \"" + oldPath +
                                "\" to \"" + newPath + "\"");
        }
      ...
      ourLogger.warn(
          "renameOrMove experienced a failure while trying to rename a file",
          t);
