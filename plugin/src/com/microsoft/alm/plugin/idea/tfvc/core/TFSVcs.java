...
public class TFSVcs extends AbstractVcs {
  ...
  @NonNls public static final String TFVC_NAME = "TFVC";
  public static final String TFVC_ONLINE_HELP_URL =
      "https://docs.microsoft.com/en-us/azure/devops/java/intellij-faq?view=azure-devops#does-the-intellij-plug-in-support-tfvc";
  ...
  private CommittedChangesProvider<TFSChangeList, ChangeBrowserSettings>
  ...
  public TFSVcs(@NotNull Project project) {
    ...
    myAddConfirmation = vcsManager.getStandardConfirmation(
        VcsConfiguration.StandardConfirmation.ADD, this);
    myDeleteConfirmation = vcsManager.getStandardConfirmation(
        VcsConfiguration.StandardConfirmation.REMOVE, this);
    myCheckoutOptions = vcsManager.getStandardOption(
        VcsConfiguration.StandardOption.CHECKOUT, this);
  }

  public static TFSVcs getInstance(Project project) {
    return (TFSVcs)ProjectLevelVcsManager.getInstance(project).findVcsByName(
        TFVC_NAME);
  }
  ...
  public void enableIntegration() {
    BackgroundTaskUtil.executeOnPooledThread(myProject, () -> {
      Collection<VcsRoot> roots =
          ServiceManager.getService(myProject, VcsRootDetector.class).detect();
  ...
  /**
   * Overrides method from IDEA 2019.2 that will allow us to work without "new"
   * root mappings.
   */
  public boolean needsLegacyDefaultMappings() { return true; }

  @NotNull
  public CommittedChangesProvider<TFSChangeList, ChangeBrowserSettings>
  ...
  public static VcsKey getKey() { return ourKey; }
   ...
   * We do not cache it here because it should already be cached in the
  ...
  public ServerContext getServerContext(boolean throwIfNotFound) {
    final RepositoryContext repositoryContext =
        VcsHelper.getRepositoryContext(getProject());
    logger.info("TFSVcs.getServerContext repositoryContext is null: " +
    ...
    if (serverContext == null && throwIfNotFound) {
      // TODO: throw a better error b/c this is what the user sees and it's
      // confusing
      throw new NotAuthorizedException(
          repositoryContext != null ? repositoryContext.getUrl() : "");
    }
  ...
  private void checkCommandLineVersion() {
    if (hasVersionBeenVerified) {
      // No need to check the version again if we have already checked it once
      // this session
      logger.info(
          "Skipping the attempt to check the version of the TF command line.");
      return;
    }

    hasVersionBeenVerified = true;

    // We want to start a background thread to check the version, but that can
    // only be done form the UI thread.
    IdeaHelper.runOnUIThread(() -> {
      final SettableFuture<String> versionMessage = SettableFuture.create();
      (new Task.Backgroundable(
           getProject(),
           TfPluginBundle.message(
               TfPluginBundle.KEY_TFVC_TF_VERSION_WARNING_PROGRESS),
           false, PerformInBackgroundOption.ALWAYS_BACKGROUND) {
        public void run(@NotNull final ProgressIndicator indicator) {
          try {
            logger.info(
                "Attempting to check the version of the TF command line.");
            TfTool.checkVersion();
            versionMessage.set(StringUtils.EMPTY);
          } catch (final ToolException ex) {
            String error =
                ToolException.KEY_TF_EXE_NOT_FOUND.equals(ex.getMessageKey())
                    ? TfPluginBundle.message(
                          TfPluginBundle
                              .KEY_TOOLEXCEPTION_TF_HOME_NOT_SET) // more
                                                                  // suitable
                                                                  // for
                                                                  // notification
                                                                  // than the
            ...
            logger.warn(
                "Unexpected error when checking the version of the command line.",
                t);
            versionMessage.set(TfPluginBundle.message(
                TfPluginBundle.KEY_TFVC_TF_CANNOT_DETERMINE_VERSION_TEXT));
          }
        }
        ...
        public void onSuccess() {
          try {
            ...
            if (StringUtils.isNotEmpty(error)) {
              ...
              // Notify the user that they should upgrade their version of the
              // TF command line
              VcsNotifier.getInstance(getProject())
            ...
            logger.warn(
                "Failed to warn user about min version of TF command line.", e);
          }
        }
      }).queue();
    });
