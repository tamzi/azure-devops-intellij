...
public class TfvcRootCache {
  ...
  private static final Logger ourLogger =
      Logger.getInstance(TfvcRootCache.class);

  private static void assertNoServiceDirectory(Path path) {
    if (TFVCUtil.isInServiceDirectory(
            new LocalFilePath(path.toString(), true))) {
      throw new InvalidPathException(
          path.toString(), "Path contains TFVC service directory name");
    }
  }
   ...
   * @return cached status or {@link CachedStatus#UNKNOWN} it no information is
  ...
  public CachedStatus get(@NotNull Path path) {
    synchronized (myLock) {
      ...
      for (Map.Entry<Path, CachedStatus> entry : myData.entrySet()) {
        ...
        // If any child of an item is known to be NO_ROOT, i.e. not under any
        // mapping, then the result is NO_ROOT.
        if (isChild && value == CachedStatus.NO_ROOT) {
          ourLogger.trace(String.format("%s: %s (derived from %s being %s)",
                                        path, value, key, value));
          return value;
        }

        // If any child of an item is known to be IS_MAPPING_ROOT, then the
        // result is NO_ROOT, since mapping roots cannot be nested.
        if (isChild && value == CachedStatus.IS_MAPPING_ROOT) {
          ourLogger.trace(String.format("%s: %s (derived from %s being %s)",
                                        path, CachedStatus.NO_ROOT, key,
                                        value));
          return CachedStatus.NO_ROOT;
        }

        // If any parent of an item is known to be IS_MAPPING_ROOT, then the
        // result is UNDER_MAPPING_ROOT.
        if (isParent && value == CachedStatus.IS_MAPPING_ROOT) {
          ourLogger.trace(String.format("%s: %s (derived from %s being %s)",
                                        path, CachedStatus.UNDER_MAPPING_ROOT,
                                        key, value));
  ...
  /**
   * Caches the fact that there're no workspace mappings for the path or its
   * parents.
   */
  public void putNoMappingsFor(@NotNull Path path) {
    ...
    synchronized (myLock) {
      // Destroy contradictory information: since we know the path isn't a
      // mapping root and contains no parent mapping roots, and information
      // about the parents being roots is now invalid.
      ...
      for (Map.Entry<Path, CachedStatus> entry : myData.entrySet()) {
        ...
        if (isParent && value == CachedStatus.IS_MAPPING_ROOT) {
          ourLogger.info(String.format(
              "Evicting information about %s being %s because %s is %s", key,
              value, path, CachedStatus.NO_ROOT));
  ...
  public void putMappings(@NotNull List<Workspace.Mapping> mappings) {
    synchronized (myLock) {
      for (Workspace.Mapping mapping : mappings) {
        ...
        // Destroy contradictory information: since we know the path is a
        // mapping root, then any information that tells us its children are
        // roots themselves or aren't under a root, or its parents are roots is
        ...
        for (Map.Entry<Path, CachedStatus> entry : myData.entrySet()) {
          ...
          if (isEvicted) {
            ourLogger.info(String.format(
                "Evicting information about %s being %s because %s is %s", key,
                value, path, CachedStatus.IS_MAPPING_ROOT));
          }
        }
  ...
  public enum CachedStatus {
    ...
    IS_MAPPING_ROOT,
    /**
     * Item is known to be under a TFVC workspace mapping root (while not itself
     * being a mapping).
     */
    UNDER_MAPPING_ROOT,
    /**
