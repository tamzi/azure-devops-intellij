...
public class TfvcCheckoutModel implements VcsSpecificCheckoutModel {
  private static final Logger logger =
      LoggerFactory.getLogger(TfvcCheckoutModel.class);
  ...
  public void doCheckout(Project project, CheckoutProvider.Listener listener,
    ...
    Workspace.Location workspaceKind = isTfvcServerCheckout
    ...
    (new Task.Backgroundable(
      ...
      public void run(@NotNull final ProgressIndicator indicator) {
        ...
        try {
          ...
          // TODO: allow user to change name in the flow instead of starting
          // over
          IdeaHelper.runOnUIThread(new Runnable() {
            ...
            public void run() {
              Messages.showErrorDialog(
                  ...
                  LocalizationServiceImpl.getInstance().getExceptionMessage(e),
                  TfPluginBundle.message(
                      TfPluginBundle.KEY_CHECKOUT_TFVC_FAILED_TITLE));
            }
          });

          // returning since the workspace failed to create so we can't proceed
          // with the next steps
          return;
        }
        ...
        IdeaHelper.setProgress(
            indicator, 0.20,
            TfPluginBundle.message(
                TfPluginBundle.KEY_CHECKOUT_TFVC_PROGRESS_ADD_ROOT));

        // Map the project root to the local folder
        final String serverPath = VcsHelper.TFVC_ROOT + teamProjectName;
        try {
          final UpdateWorkspaceMappingCommand mappingCommand =
          ...
          final DeleteWorkspaceCommand deleteWorkspaceCommand =
        ...
        IdeaHelper.setProgress(
            indicator, 0.30,
            TfPluginBundle.message(
                TfPluginBundle.KEY_CHECKOUT_TFVC_PROGRESS_CREATE_FOLDER));
        ...
        // if advanced is set, then sync just some of the files (those that we
        // need for IntelliJ) Otherwise, sync all the files for the team project
        if (!isAdvancedChecked) {
          IdeaHelper.setProgress(
              indicator, 0.50,
              TfPluginBundle.message(
                  TfPluginBundle.KEY_CHECKOUT_TFVC_PROGRESS_SYNC));
        ...
        if (checkoutResult.get()) {
          ...
          if (isAdvancedChecked) {
            ...
            final RepositoryContext repositoryContext =
                RepositoryContext.createTfvcContext(localPath, workspaceName,
                                                    teamProjectName,
                                                    context.getServerUri());
            final WorkspaceController controller = new WorkspaceController(
                project, repositoryContext, context, workspaceName);
            if (controller.showModalDialog(false)) {
              ...
              controller.saveWorkspace(localPath, true, new Runnable() {
                ...
                public void run() {
                  ...
                  UpdateVersionControlSystem(project, parentDirectory,
                                             directoryName, destinationParent,
                                             listener);
                }
              });
            }
          } else {
            // We don't have to wait for the workspace to be updated, so just
            // trigger the VCS update
  ...
  private void UpdateVersionControlSystem(final Project project,
    ...
    DvcsUtil.addMappingIfSubRoot(
        project, FileUtil.join(new String[] {parentDirectory, directoryName}),
        TFSVcs.TFVC_NAME);
  ...
  public String getButtonText() {
    return TfPluginBundle.message(
        TfPluginBundle.KEY_CHECKOUT_DIALOG_CREATE_WORKSPACE_BUTTON);
  }
