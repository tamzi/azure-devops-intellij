...
public abstract class SingleItemAction extends DumbAwareAction {
  protected static final Logger logger =
      LoggerFactory.getLogger(SingleItemAction.class);

  private static final Collection<FileStatus> ALLOWED_STATUSES =
      Arrays.asList(FileStatus.HIJACKED, FileStatus.MODIFIED,
                    FileStatus.NOT_CHANGED, FileStatus.OBSOLETE);
  ...
  execute(final @NotNull SingleItemActionContext actionContext)
      throws TfsException;
  ...
  public void actionPerformed(final AnActionEvent e) {
    ...
    final String actionTitle =
        StringUtil.trimEnd(e.getPresentation().getText(), "...");
    try {
      final SingleItemActionContext actionContext =
          new SingleItemActionContext(project, localPath);
      ...
      ProgressManager.getInstance().runProcessWithProgressSynchronously(() -> {
        try {
          ...
          final ItemInfo item =
              CommandUtils.getItemInfo(context, localPath.getPath());
  ...
  public void update(@NotNull final AnActionEvent e) {
    e.getPresentation().setEnabled(
        isEnabled(e.getProject(), VcsUtil.getOneVirtualFile(e)));
  }
  ...
  protected final boolean isEnabled(@Nullable final Project project,
                                    @Nullable final VirtualFile file) {
    if (project == null || file == null ||
        !getAllowedStatuses().contains(
            FileStatusManager.getInstance(project).getStatus(file)))
      return false;

    // TODO: Remove this suppression after migration to IDEA 2019.3. AbstractVcs
    // became non-generic in newer IDEA.
  ...
  public static class SingleItemActionContext {
    ...
    public SingleItemActionContext(final Project project,
    ...
    public ServerContext getServerContext() { return serverContext; }
  }
}
