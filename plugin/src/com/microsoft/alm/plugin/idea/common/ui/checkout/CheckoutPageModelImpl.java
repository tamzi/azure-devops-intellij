...
/**
 * This class is provided as a base for the VSO and TFS models. It provides the
 * majority of the functionality with a few abstract methods that must be
 * overridden.
 */
public abstract class CheckoutPageModelImpl extends LoginPageModelImpl
    implements CheckoutPageModel, ServerContextLookupPageModel {
  ...
  public CheckoutPageModelImpl(final CheckoutModel checkoutModel,
                               final ServerContextTableModel.Column[] columns) {
    super(checkoutModel);
    ...
    parentDirectory =
        PluginServiceProvider.getInstance().getPropertyService().getProperty(
            PropertyService.PROP_REPO_ROOT);
  ...
  /**
   * This getter allows the derived classes to access the CheckoutModel that
   * owns them.
   */
  protected CheckoutModel getParentModel() { return parentModel; }
  ...
  public boolean isTfvcServerCheckout() { return isTfvcServerCheckout; }
  ...
  public ModelValidationInfo validate() {
    ...
    if (result == ModelValidationInfo.NO_ERRORS) {
      ...
      // We test this method and so we need to check to see if we are in
      // IntelliJ before using VirtualFileManager ApplicationManager is null if
      // we are not in IntelliJ
      if (ApplicationManager.getApplication() != null) {
        final VirtualFile destinationParent =
            LocalFileSystem.getInstance().findFileByPath(parentDirectory);
      ...
      if (destDirectoryOnDisk.exists() && destDirectoryOnDisk.isDirectory()) {
        return ModelValidationInfo.createWithResource(
            PROP_DIRECTORY_NAME,
            TfPluginBundle.KEY_CHECKOUT_DIALOG_ERRORS_DESTINATION_EXISTS,
            directoryName);
      }
      // verify destination directory parent exists, we can reach this condition
      // if user specifies a path for directory name
      if (destDirectoryOnDisk.getParentFile() == null ||
          !destDirectoryOnDisk.getParentFile().exists()) {
        return ModelValidationInfo.createWithResource(
            PROP_DIRECTORY_NAME,
  ...
  @Override public abstract void loadRepositories();
  ...
  public void cloneSelectedRepo() {
    ...
    if (validationInfo == null) {
      ...
      parentModel.doCheckout(
          getParentModel().getProject(), getParentModel().getListener(),
          context, destinationParent, getDirectoryName(), getParentDirectory(),
          isAdvanced(), isTfvcServerCheckout);

      // Save parent directory for next time
      PluginServiceProvider.getInstance().getPropertyService().setProperty(
          PropertyService.PROP_REPO_ROOT, getParentDirectory());
    }
  }

  /**
   * This method is provided to allow the derived classes an easy way to get the
  ...
  /**
   * This method is provided to allow the listener to update the list of
   * contexts.
   */
  @Override
  ...
  /**
   * This method is provided to allow the derived classes an easy way to add to
  ...
  private void setupSelectionListener() {
    // Set up event listener to set the Directory name when the selection
    // changes
    repositoryTableModel.getSelectionModel().addListSelectionListener(
        new ListSelectionListener() {
          @Override
          public void valueChanged(ListSelectionEvent e) {
            // No need to change things while the list is adjusting
            if (!e.getValueIsAdjusting()) {
              final ServerContext row =
                  repositoryTableModel.getSelectedContext();
