...
public class LocalizationServiceImpl implements LocalizationService {
  ...
  public String getExceptionMessage(Throwable t) {
    ...
    if (StringUtils.isEmpty(message) && t.getCause() != null) {
      if (t.getCause() instanceof LocalizedException) {
        final LocalizedException localizedException =
            (LocalizedException)t.getCause();
        final String key = localizedException.getMessageKey();
        if (keysMap.containsKey(key)) {
          message = getLocalizedMessage(
              keysMap.get(key),
    ...
    // No message on the exception and the cause, just use description from
    // toString
  ...
  private static final Map<String, String> keysMap = new HashMap<String,
                                                                 String>() {
    {
      // Exception messages
      put(TeamServicesException.KEY_TFS_UNSUPPORTED_VERSION,
          "TFS.UnsupportedVersion");
      put(TeamServicesException.KEY_VSO_AUTH_SESSION_EXPIRED,
          "VSO.Auth.SessionExpired");
      ...
      put(TeamServicesException.KEY_OPERATION_ERRORS,
          "Operation.Lookup.Errors");
      put(TeamServicesException.KEY_VSO_NO_PROFILE_ERROR, "VSO.NoProfileError");
      put(TeamServicesException.KEY_TFS_MALFORMED_SERVER_URI,
          "TFS.MalformedServerUri");
      put(TeamServicesException.KEY_ERROR_UNKNOWN, "Errors.Unknown");
      put(TeamServicesException.KEY_TFS_SERVER_PATH_INVALID,
          "TFS.ServerPath.Invalid");
      ...
      put(ToolException.KEY_TF_HOME_NOT_SET,
          TfPluginBundle.KEY_TOOLEXCEPTION_TF_HOME_NOT_SET);
      ...
      put(ToolException.KEY_TF_MIN_VERSION_WARNING,
          "ToolException.TF.MinVersionWarning");
      put(ToolException.KEY_TF_WORKSPACE_COULD_NOT_BE_DETERMINED,
          "ToolException.TF.WorkspaceCouldNotBeDetermined");
      put(ToolException.KEY_TF_WORKSPACE_EXISTS,
          "ToolException.TF.WorkspaceExists");
      ...
      put(ToolException.KEY_TF_VS_MIN_VERSION_WARNING,
          "ToolException.VS.MinVersionWarning");
    }
  };
}
