...
public class ServerContextManager {
  private static final Logger logger =
      LoggerFactory.getLogger(ServerContextManager.class);

  private final String CONNECTION_DATA_REST_API_PATH =
      "/_apis/connectionData?connectOptions=IncludeServices&lastChangeId=-1&lastChangeId64=-1&api-version=1.0";
  private final String TFS2015_NEW_SERVICE = "distributedtask";

  private Map<String, ServerContext> contextMap =
      new HashMap<String, ServerContext>();

  private static class Holder {
    private static final ServerContextManager INSTANCE =
        new ServerContextManager(true);
  }

  /**
   * The constructor is protected for tests.
   */
  protected ServerContextManager() { this(false); }
  ...
  public static ServerContextManager getInstance() { return Holder.INSTANCE; }
  ...
  private void setLastUsedContextKey(String key) {
    PluginServiceProvider.getInstance().getPropertyService().setProperty(
        PropertyService.PROP_LAST_CONTEXT_KEY, key);
  }

  private String getLastUsedContextKey() {
    return PluginServiceProvider.getInstance().getPropertyService().getProperty(
        PropertyService.PROP_LAST_CONTEXT_KEY);
  }
  ...
  private boolean shouldBeSaved(final ServerContext context) {
    ...
    if (shouldBeSaved && context.getAuthenticationInfo() != null &&
        AuthenticationInfo.CredsType.AccessToken.equals(
            context.getAuthenticationInfo().getType())) {
  ...
  public synchronized Collection<ServerContext> getAllServerContexts() {
    ...
    return Collections.unmodifiableCollection(
        new ArrayList<ServerContext>(contextMap.values()));
  }
  ...
  /**
   * Validates a provided server context and if validation succeeds saves a
   * server context with the user's team foundation Id
   *
   * @param context
   */
  ...
  protected ServerContext validateServerConnection(final ServerContext context,
                                                   Validator validator) {
    ...
    if (UrlHelper.isGitRemoteUrl(context.getUri().toString())) {
      ...
      // Assume this is a TFVC url and parse the TFVC url to get collection
      // information
      if (validator.validateTfvcUrl(context.getUri().toString(),
    ...
    logger.info("Validating a {} server context with a server URI {}",
                context.getType(),
                contextToValidate == null ? ""
                                          : contextToValidate.getServerUri());
  ...
  private ServerContext checkVstsConnection(final ServerContext context)
      throws TeamServicesException {
    final String CONNECTION_DATA_REST_API_PATH =
        "/_apis/connectionData?connectOptions=lastChangeId=-1&lastChangeId64=-1&api-version=1.0";

    if (context == null || context.getServerUri() == null) {
      throw new TeamServicesException(
          TeamServicesException.KEY_VSO_AUTH_FAILED);
    }

    final ConnectionData data = VstsHttpClient.sendRequest(
        context.getClient(),
    ...
    final ServerContext contextWithUserId =
        new ServerContextBuilder(context)
  ...
  checkTfsVersionAndConnection(final ServerContext context)
      throws TeamServicesException {
    if (context == null || context.getServerUri() == null) {
      throw new TeamServicesException(
          TeamServicesException.KEY_TFS_AUTH_FAILED);
    }
    ...
    try {
      ...
      if (data.getLocationServiceData() != null &&
        ...
        for (final ServiceDefinition s :
             data.getLocationServiceData().getServiceDefinitions()) {
          if (StringUtils.equalsIgnoreCase(s.getServiceType(),
                                           TFS2015_NEW_SERVICE)) {
            ...
            final ServerContext contextWithUserId =
                new ServerContextBuilder(context)
                    ...
                    .build();
            add(contextWithUserId);
            final ServerContext lastUsedTfsContext =
                new ServerContextBuilder(contextWithUserId)
                    .uri(TfsAuthenticationProvider.TFS_LAST_USED_URL)
                    .build();
        ...
        // This is TFS 2013
        logger.warn(
            "checkTfsVersionAndConnection: Detected an attempt to connect to a TFS 2013 server");

        throw new TeamServicesException(
            TeamServicesException.KEY_TFS_UNSUPPORTED_VERSION);
      }
    } catch (com.microsoft.alm.plugin.context.rest.VstsHttpClient
                 .VstsHttpClientException e) {
      if (e.getStatusCode() == 404) {
        // HTTP not found, so server does not have this endpoint (TFS 2012 or
        // older) or the URL is incorrect
        logger.warn(
            "checkTfsVersionAndConnection: 404 while trying to connect to server so either bad url or unsupported server version");
        throw new TeamServicesException(
            TeamServicesException.KEY_TFS_UNSUPPORTED_VERSION);
      } else {
   ...
   * Note that if a context does not exist, one will be created and the user
   * will be prompted if necessary. Run this on a background thread, will hang
   * if run on the UI thread
   */
  public ServerContext
  getAuthenticatedContext(final String gitRemoteUrl,
                          final boolean setAsActiveContext) {
    try {
      // get context from builder, create PersonalAccessToken if needed, and
   ...
   * Note that this will require server calls and should be done on a background
   * thread.
   *
   * @param gitRemoteUrl
   * @return
   */
  public ServerContext
  createContextFromGitRemoteUrl(final String gitRemoteUrl) {
  ...
  public ServerContext createContextFromGitRemoteUrl(final String gitRemoteUrl,
    ...
    final String httpsGitRemoteUrl =
        UrlHelper.getHttpsGitUrlFromSshUrl(gitRemoteUrl);
    ...
    if (context == null || context.getGitRepository() == null ||
        ...
        !StringUtils.equalsIgnoreCase(context.getUsableGitUrl(),
                                      httpsGitRemoteUrl)) {
      context = null;
    }
    ...
    if (context == null) {
      ...
      final AuthenticationInfo authenticationInfo =
          getAuthenticationInfo(httpsGitRemoteUrl, prompt);
      if (authenticationInfo != null) {
        final ServerContext.Type type =
            UrlHelper.isTeamServicesUrl(httpsGitRemoteUrl)
                ? ServerContext.Type.VSO
                : ServerContext.Type.TFS;
        final ServerContext contextToValidate =
            new ServerContextBuilder()
                .type(type)
                .uri(httpsGitRemoteUrl)
  ...
  public ServerContext
  createContextFromTfvcServerUrl(@NotNull URI tfvcServerUrl,
                                 String teamProjectName, boolean prompt) {
    String tfvcServerUrlString = tfvcServerUrl.toString();

    // Get matching context from manager
    ServerContext context = get(tfvcServerUrlString);
    logger.info("createContextFromTfvcServerUrl context exists: " +
                (context != null));
    if (context == null || context.getServerUri() == null ||
        context.getTeamProjectCollectionReference() == null ||
        ...
        !StringUtils.equalsIgnoreCase(
            context.getTeamProjectReference().getName(), teamProjectName)) {
      context = null;
      logger.info("createContextFromTfvcServerUrl context fully populated: " +
                  (context != null));
    }
    ...
    if (context == null) {
      // Manager didn't have a matching context, so try to look up the auth info
      final AuthenticationInfo authenticationInfo =
          getAuthenticationInfo(tfvcServerUrl, prompt);
      if (authenticationInfo != null) {
        final ServerContext.Type type =
            UrlHelper.isTeamServicesUrl(tfvcServerUrlString)
                ? ServerContext.Type.VSO
                : ServerContext.Type.TFS;
        ...
        final ServerContext contextToValidate =
            new ServerContextBuilder()
                ...
                .build();
        logger.info("type = " + type + ", uri = " + tfvcServerUrl +
                    ", projectName = " + teamProjectName);
        try {
          context = validateServerConnection(contextToValidate);
        } catch (NotAuthorizedException e) {
          logger.warn(
              "createContextFromTfvcServerUrl: NotAuthorizedException occurred");
          if (prompt) {
            // refreshing creds
            logger.info("createContextFromTfvcServerUrl: refreshing creds");
            updateAuthenticationInfo(tfvcServerUrlString);
            final AuthenticationInfo authenticationInfoRefreshed =
                getAuthenticationInfo(tfvcServerUrl, prompt);
            final ServerContext contextToValidateRefreshed =
                new ServerContextBuilder()
                    .type(type)
                    .uri(tfvcServerUrl)
                    .authentication(authenticationInfoRefreshed)
                    .teamProject(teamProjectName)
                    .build();
            context = validateServerConnection(contextToValidateRefreshed);
          } else {
            logger.info(
                "createContextFromTfvcServerUrl: Stale creds detected but feature does not allow prompting for refresh");
          }
        }
      } else {
        logger.warn(
            "createContextFromTfvcServerUrl: authentication info returned null");
      }
    }
  ...
  /**
   * This method tries to find existing authentication info for a given server
   * URI. If the auth info cannot be found and the prompt flag is true, the user
   * will be prompted.
   */
  @Nullable
  public AuthenticationInfo getBestAuthenticationInfo(@NotNull URI uri,
                                                      boolean prompt) {
   ...
   * If the auth info cannot be found and the prompt flag is true, the user will
   * be prompted.
   *
   * @deprecated Use {@link #getBestAuthenticationInfo(URI, boolean)} instead.
   */
  public AuthenticationInfo getBestAuthenticationInfo(final String url,
                                                      final boolean prompt) {
   ...
   */
  public AuthenticationInfo getAuthenticationInfo(String gitRemoteUrl,
                                                  boolean prompt) {
   ...
   * will be prompted.
   */
  public AuthenticationInfo getAuthenticationInfo(URI serverUri,
                                                  final boolean prompt) {
    AuthenticationInfo authenticationInfo = null;

    // For now I will just do a linear search for an appropriate context info to
    // copy the auth info from
    ...
    if (authenticationInfo == null && prompt) {
      ...
      final AuthenticationProvider authenticationProvider =
          getAuthenticationProvider(uriString);
      authenticationInfo = AuthHelper.getAuthenticationInfoSynchronously(
          authenticationProvider, uriString);
    }
  ...
  /**
   * Takes existing contexts, creates new auth info for them, and creates new
   * contexts with that info and all other contexts that share that auth info
   * and removes the old contexts from use
   *
   * @param contexts
   * @return
   */
  public void updateServerContextsAuthInfo(final List<ServerContext> contexts) {
    logger.info("updateServerContextsAuthInfo: starting with context count: " +
                contexts.size() + ", contextMap size: " + contextMap.size());
    final HashMap<AuthenticationInfo, AuthenticationInfo> authInfoMap =
        new HashMap<AuthenticationInfo, AuthenticationInfo>(contexts.size());
    ...
    logger.info("updateServerContextsAuthInfo: ending with contextMap size: " +
                contextMap.size());
  }

  /**
   * Checks all contexts for old auth info and creates new contexts with the new
   * auth info
   *
   * @param authInfoMap: keys are old auth infos and the values are the new auth
   *     infos
   */
  private void refreshAuthInfo(
    ...
    for (final ServerContext context : getAllServerContexts()) {
      final AuthenticationInfo contextAuthInfo =
          context.getAuthenticationInfo();
      if (authInfoMap.containsKey(contextAuthInfo)) {
        logger.info("refreshAuthInfo: Refreshing context auth info for: " +
                    context.getKey());
        remove(context.getKey());
        add(new ServerContextBuilder(context)
                .authentication(authInfoMap.get(contextAuthInfo))
                .build());
      }
    }
  }
  ...
  /**
   * Takes an existing server context and creates a new server context from it
   * with everything the same except it prompts the user for new authentication
   * info.
   *
  ...
  private AuthenticationInfo getNewAuthInfo(final ServerContext context) {
    ...
    final AuthenticationProvider authenticationProvider =
        ServerContext.Type.TFS.equals(context.getType())
            ? TfsAuthenticationProvider.getInstance()
            : VsoAuthenticationProvider.getInstance();
    // can't determine if TFVC or Git from context so check for a Git URL
    String url = context.getUsableGitUrl() == null ? context.getUri().toString()
                                                   : context.getUsableGitUrl();
    // if the URL is the default value we give for last used get it from the
    // auth info instead
    if (StringUtils.equals(url, TfsAuthenticationProvider.TFS_LAST_USED_URL) &&
    ...
    AuthenticationInfo newAuthInfo =
        AuthHelper.getAuthenticationInfoSynchronously(authenticationProvider,
                                                      url);
    if (newAuthInfo != null) {
      // Creating a new authentication info object will replace the existing
      // context with a newly constructed one that doesn't preserve the
      // repository information. To fix that, we'll create a new context using
      // the existing context information, and put it into the manager.
      add(new ServerContextBuilder(context)
              .authentication(newAuthInfo)
              .build());
    }
  ...
  /**
   * Gets back the most updated context with auth info possible. It first checks
   * to see if an existing context exists and if not it tries to create one. If
   * the create fails (possibility auth info used was stale) then the auth info
   * is updated and then we try to create the context again
   * <p/>
   * TODO: Rip out this method and refactor the code to throw up the
   * unauthorized exception instead of swallowing it
  ...
  public ServerContext getUpdatedContext(final String remoteUrl,
    ...
    ServerContext context =
        getAuthenticatedContext(remoteUrl, setAsActiveContext);
    ...
    // if the context was not obtained in the first try, update the auth info
    // and try again if need be
    context = updateAuthenticationInfo(remoteUrl);
    logger.info("getUpdatedContext updated auth info and found a context: " +
  ...
  /**
   * Updates all contexts with matching authority in URI with new authentication
   * info, will prompt the user Has to be called on a background thread, will
   * hang if called on UI thread
   *
   * @param remoteUrl
   */
  public ServerContext updateAuthenticationInfo(final String remoteUrl) {
    ...
    // Linear search through all contexts to find the ones with same authority
    // as remoteUrl
    for (final ServerContext context : getAllServerContexts()) {
      logger.info("auth info updateAuthenticationInfo compare " +
      ...
      if (UrlHelper.haveSameAccount(remoteUri, context.getUri())) {
        ...
        if (promptUser) {
          ...
          // prompt user
          final AuthenticationProvider authenticationProvider =
              getAuthenticationProvider(remoteUrl);
          authenticationProvider.clearAuthenticationDetails(
              context.getServerUri().toString());
          newAuthenticationInfo = AuthHelper.getAuthenticationInfoSynchronously(
              authenticationProvider, remoteUrl);
          promptUser = false;
        }
        ...
        if (newAuthenticationInfo != null) {
          ...
          if (StringUtils.equalsIgnoreCase(context.getUri().toString(),
                                           remoteUrl)) {
            logger.info(
                "The updated auth info created a context that matches the remote url");
            add(newContext, true);
            matchingContext = newContext;
          } else {
            logger.info(
                "The updated auth info created a context that has a different remote url");
    ...
    logger.info(
        "auth info updateAuthenticationInfo returning an updated context: " +
        (matchingContext == null ? "false" : "true"));
    return matchingContext;
  }

  /**
   * Use this method to get the appropriate AuthenticationProvider based on an
   * url.
   *
   * @param url
   * @return
  ...
  protected static class Validator implements UrlHelper.ParseResultValidator {
    ...
    public Validator(final ServerContext context) { this.context = context; }

    public String getServerUrl() { return serverUrl; }

    public GitRepository getRepository() { return repository; }

    public TeamProjectCollection getCollection() { return collection; }

    public TeamProjectReference getProject() { return project; }

    protected GitHttpClient getGitHttpClient(final Client jaxrsClient,
                                             final URI baseUrl) {
      return new GitHttpClient(jaxrsClient, baseUrl);
    }

    protected CoreHttpClient getCoreHttpClient(final Client jaxrsClient,
                                               final URI baseUrl) {
      return new CoreHttpClient(jaxrsClient, baseUrl);
    }

    protected TeamProjectCollection
    getCollectionFromServer(final ServerContext context,
                            String collectionName) {
      final TeamProjectCollectionReference ref =
          context.getSoapServices().getCatalogService().getProjectCollection(
              collectionName);
    ...
    protected TeamProjectReference
    getProjectFromServer(final ServerContext context, URI collectionURI,
                         String teamProjectName) {
      final CoreHttpClient client =
          new CoreHttpClient(context.getClient(), collectionURI);
    ...
    /**
     * This method queries the server with the given Git remote URL for
     * repository, project and collection information If unable to get the info,
     * it parses the Git remote url and tries to verify it by querying the
     * server again
     *
     * @param gitRemoteUrl
    ...
    public boolean validateGitUrl(final String gitRemoteUrl) {
      try {
        ...
        if (UrlHelper.isSshGitRemoteUrl(gitRemoteUrl)) {
          ...
          // Trim user info from the URI (e.g. https://username@dev.azure.com →
          // https://dev.azure.com, because otherwise the HTTP client will try
          // to use authentication data from the URL instead of the data baked
        ...
        final UrlHelper.ParseResult uriParseResult =
            UrlHelper.tryParse(gitUrlToParse, this);
      ...
      logger.info(
          "validateGitUrl: failed to get Azure DevOps repo, project and collection info");
      return false;
    }
    ...
    private boolean getVstsInfoForGit(final String gitRemoteUrl) {
      try {
        ...
        final VstsInfo vstsInfo = VstsHttpClient.sendRequest(
            context.getClient(), gitRemoteUrl.concat(REPO_INFO_URL_PATH),
            VstsInfo.class);
        ...
        // failed to get VSTS information, endpoint may not be available on the
        // server
        ...
        if (AuthHelper.isNotAuthorizedError(throwable)) {
          throw new TeamServicesException(
              TeamServicesException.KEY_VSO_AUTH_FAILED, throwable);
        }
        return false;
      }
    }
    ...
    /**
     * This method queries the server with the given TFVC URL for collection
     * information
     *
     * @param collectionUrl
     ...
     * @return true if server information is determined
     */
    public boolean validateTfvcUrl(final String collectionUrl,
                                   final String teamProjectName,
                                   final String possibleCollectionName) {
      try {
        final String collectionName;
        ...
        if (UrlHelper.isTeamServicesUrl(collectionUrl)) {
          if (UrlHelper.isOrganizationUrl(collectionUrl)) {
            ...
            // The Team Services collection is ALWAYS defaultCollection, and
            // both the url with defaultcollection and the url without
            // defaultCollection will validate just fine. However, it expects
            // you to refer to the collection by the account name. So, we just
            // need to grab the account name and use that to recreate the url.
            // If validation fails, we return false.
            final String accountName =
                UrlHelper.getVSOAccountName(UrlHelper.createUri(collectionUrl));
          ...
          // A full Team Foundation Server collection url is required for the
          // validate call to succeed. So, we try the url given. If that fails,
          // we assume it is a server Url and the collection is the
          ...
          if (validateTfvcCollectionUrl(collectionUrl)) {
            ...
            if (!validateTfvcCollectionUrl(
                    UrlHelper
                        .getCollectionURI(UrlHelper.createUri(serverUrl),
                                          collectionName)
                        .toString())) {
              return false;
            }
        ...
        // Get the collection object from the server (different based on VSO vs
        // OnPrem)
        if (UrlHelper.isTeamServicesUrl(serverUrl)) {
          final CoreHttpClient coreClient = getCoreHttpClient(
              context.getClient(), UrlHelper.createUri(serverUrl));
          collection = coreClient.getProjectCollection(collectionName);
        } else {
          final ServerContext contextToValidate =
              new ServerContextBuilder(context).serverUri(serverUrl).build();
          collection =
              getCollectionFromServer(contextToValidate, collectionName);
        }
        // Get the Team Project object from the server
        this.project = getProjectFromServer(
            context,
            UrlHelper.getCollectionURI(UrlHelper.createUri(serverUrl),
                                       collectionName),
            teamProjectName);
        return true;
    ...
    private boolean validateTfvcCollectionUrl(final String collectionUrl) {
      // Try to query the server endpoint for branches to see if the collection
      // url is correct
      try {
        VstsHttpClient.sendRequest(
            context.getClient(),
            UrlHelper.combine(collectionUrl, TFVC_BRANCHES_URL_PATH),
            String.class);
        logger.info("validateTfvcCollectionUrl: validated url: " +
                    collectionUrl);
        return true;
      } catch (VstsHttpClient.VstsHttpClientException e) {
        if (e.getStatusCode() == 404) {
          // Add the DefaultCollection to the url and try again
          logger.info("validateTfvcCollectionUrl: found 404 for url: " +
                      collectionUrl);
          return false;
        } else {
    ...
    /**
     * This method gets all the info we need from the server given the parse
     * results. If some call fails we simply return false and ignore the
     * results.
     *
     * @param parseResult
     * @return
    ...
    public boolean validate(final UrlHelper.ParseResult parseResult) {
      try {
        ...
        final URI collectionUri =
            UrlHelper.createUri(parseResult.getCollectionUrl());
        final GitHttpClient gitClient =
            getGitHttpClient(context.getClient(), collectionUri);
        // Get the repository object and team project
        repository = gitClient.getRepository(parseResult.getProjectName(),
                                             parseResult.getRepoName());
        ...
        if (UrlHelper.isTeamServicesUrl(parseResult.getServerUrl())) {
          final CoreHttpClient coreClient =
              getCoreHttpClient(context.getClient(), serverUri);
          collection =
              coreClient.getProjectCollection(parseResult.getCollectionName());
        } else {
          final ServerContext contextToValidate =
              new ServerContextBuilder(context).serverUri(serverUrl).build();
          collection = getCollectionFromServer(contextToValidate,
                                               parseResult.getCollectionName());
        }
      } catch (Throwable throwable) {
        logger.error("validate: failed for parseResult " +
