...
public class UrlHelper {
  ...
  // this formatter assumes the entire text is a hyperlink, works for short
  // messages
  public static final String SHORT_HTTP_LINK_FORMATTER =
      "<html><body><a href=\"%s\">%s</a></body></html>";

  public static final String HOST_VSO = "visualstudio.com";
  public static final String HOST_TFS_ALL_IN =
      "tfsallin.net"; // azure test subscriptions
  ...
  private static final String URL_BUILD_SPECIFIC_ITEM_PATH_SEGMENT =
      "?buildid=%d&_a=summary";
  private static final String URL_BUILD_ASPX_SEGMENT =
      "web/build.aspx?pcguid=%s";
  private static final String URL_BUILD_TEAM_PROJECT_SEGMENT =
      "&projectname=%s";
  private static final String URL_BUILD_DEFINITION_ID_SEGMENT =
      "&definitionid=%d";
  ...
  private static final String URL_WIT_SPECIFIC_ITEM_PATH_SEGMENT =
      "?id=%d&_a=edit";
  private static final String URL_BRANCH_SEGMENT =
      "?path=%2F&_a=contents&version=GB";
  ...
  protected static final String URL_TFVC_ANNOTATE_FILE_SEGMENT =
      "?path=%s&_a=contents&annotate=true&hideComments=true";
  protected static final String URL_TFVC_CHANGESET_SEGMENT = "changeset";

  private static final String HTTP_PROTOCOL = "http";
  private static final String HTTPS_PROTOCOL = "https";
  private static final String DEFAULT_TEAM_SERVICES_LINK =
  ...
  public static String trimTrailingSeparators(final String uri) {
    if (StringUtils.isNotEmpty(uri)) {
      ...
      while (lastIndex > 0 &&
             uri.charAt(lastIndex - 1) == URL_SEPARATOR.charAt(0)) {
        lastIndex--;
      }
      if (lastIndex >= 0) {
  ...
  public static String trimLeadingAndTrailingSeparators(final String uri) {
    if (StringUtils.isNotEmpty(uri)) {
      int startingIndex = 0;
      while (startingIndex < uri.length() &&
             uri.charAt(startingIndex) == URL_SEPARATOR.charAt(0)) {
        startingIndex++;
      }
      int lastIndex = uri.length();
      while (lastIndex > 0 &&
      ...
      if (startingIndex < uri.length() && lastIndex >= 0 &&
  ...
  /**
   * Removes any user information from an URL, e.g. will remove the "username@"
   * part from the URL "https://username@dev.azure.com/".
   */
  ...
  @Nullable
  public static URI createOrganizationUri(@NotNull String host,
                                          @NotNull String organization) {
    URI uri = URI.create("https://" + host);
    if (!UriHelper.isAzureHost(uri)) {
      logger.error("Non-azure host passed to convertToOrganizationUrl: {}",
                   host);
      return null;
    }
  ...
  public static String getAccountFromOrganizationUri(final URI uri) {
    // Organization should be in the form azure.com/account1, we ignore any
    // other parameters
    ...
    if (pathSegments.length > 0) {
      ...
      logger.debug(
          "getAccountFromOrganizationUri: expected path for organization account = " +
          uri);
    }
    return null;
  }
  ...
  public static boolean isSshGitRemoteUrl(final String gitRemoteUrl) {
    if (isGitRemoteUrl(gitRemoteUrl) && isTeamServicesUrl(gitRemoteUrl)) {
      ...
      // check for @ in url - team project name, repo name, collection name and
      // account name don't allow @ E.g of valid url formats:
  ...
  public static String getHttpsUrlFromHttpUrl(final String httpUrl) {
    ...
    if (uri != null && StringUtils.equalsIgnoreCase(uri.getScheme(), "http")) {
      final URI httpsUri =
          createUri("https://" + uri.getAuthority() + uri.getPath());
  ...
  public static String getHttpsGitUrlFromSshUrl(final String sshGitRemoteUrl) {
    ...
    } else if (StringUtils.startsWithIgnoreCase(sshGitRemoteUrl,
                                                HTTP_PROTOCOL) ||
               StringUtils.startsWithIgnoreCase(sshGitRemoteUrl,
                                                HTTPS_PROTOCOL)) {
      // http/https url so return the url untouched
      return sshGitRemoteUrl;
    } else {
      // unsure what this url is
      logger.warn("getHttpsGitUrlFromSshUrl: can't determine https url from " +
                  sshGitRemoteUrl);
      return null;
    }
  }
  ...
  public static URI getVSOAccountURI(final String accountName) {
    return UrlHelper.createUri(
        "https://" + accountName + "." +
        HOST_VSO); // TODO: how to get account url correctly?
  }

  public static URI getCollectionURI(final URI serverUri,
                                     final String collectionName) {
    if (isOrganizationURI(serverUri) ||
        isVSO(serverUri) &&
            getVSOAccountURI(collectionName).equals(serverUri)) {
      // Either we are using an organization where the collection is the same as
      // the server uri or collection in the domain case on VSTS on the old
      // style visualstudio.com domain
      return serverUri;
    }
    return UrlHelper.createUri(combine(serverUri.toString(), collectionName));
  }

  public static URI getTeamProjectURI(final URI serverUri,
                                      final String collectionName,
                                      final String teamProjectName) {
    return UrlHelper.createUri(
        combine(getCollectionURI(serverUri, collectionName).toString(),
                teamProjectName));
  }

  public static URI getBuildsPageURI(final URI projectUri) {
    return UrlHelper.createUri(
        combine(projectUri.toString(), URL_BUILD_PATH_SEGMENT));
  }
  ...
  public static URI getQueueBuildURI(final URI serverUri,
                                     final String collectionId,
                                     final String projectName,
                                     final int buildDefinitionId) {
    return UrlHelper.createUri(
        combine(serverUri.toString(),
                String.format(URL_BUILD_ASPX_SEGMENT, collectionId))
            .concat(String.format(URL_BUILD_TEAM_PROJECT_SEGMENT,
                                  encode(projectName)))
            .concat(String.format(URL_BUILD_DEFINITION_ID_SEGMENT,
                                  buildDefinitionId))
            .concat(URL_BUILD_QUEUE_ACTION));
  }

  public static URI getCreateWorkItemURI(final URI projectUri) {
    // TODO: this url isn't exactly correct because we don't know the WI Type to
    // create
    return UrlHelper.createUri(
        combine(projectUri.toString(), URL_WIT_PATH_SEGMENT));
  }

  public static URI getSpecificWorkItemURI(final URI projectUri,
  ...
  public static URI getWorkItemRefURI(final URI teamProjectUri,
                                      final String workItemId) {
    return UrlHelper.createUri(
        combine(teamProjectUri.toString(), URL_WIT_REF_SEGMENT, workItemId));
  }

  public static URI getMyWorkItemsURI(final URI projectUri) {
    // The default query when you navigate to the work items section is the
    // "Assigned to me" query results
    return UrlHelper.createUri(
        combine(projectUri.toString(), URL_WIT_PATH_SEGMENT));
  }

  public static URI getBranchURI(final URI repoUri, final String branchName) {
    return UrlHelper.createUri(repoUri.toString()
                                   .concat(URL_BRANCH_SEGMENT)
                                   .concat(encode(branchName)));
  }

  public static URI getCommitURI(final String remoteUrl,
                                 final String commitId) {
    return UrlHelper.createUri(combine(getHttpsGitUrlFromSshUrl(remoteUrl),
                                       URL_COMMIT_SEGMENT, commitId));
  }

  public static URI getFileURI(final String remoteUrl, final String filePath,
                               final String gitRemoteBranchName) {
    String uri = getHttpsGitUrlFromSshUrl(remoteUrl)
                     .concat(URL_PATH_SEGMENT)
                     .concat(filePath);
  ...
  public static URI getTfvcChangesetURI(final String remoteUrl,
                                        final String changesetNumber) {
    return UrlHelper.createUri(
        combine(getHttpsGitUrlFromSshUrl(remoteUrl), URL_TFVC_PATH_SEGMENT,
                URL_TFVC_CHANGESET_SEGMENT, changesetNumber));
  }

  public static URI getTfvcAnnotateURI(final String collectionUrl,
                                       final String projectName,
                                       final String filePath) {
    return createUri(combine(
        collectionUrl, projectName, URL_TFVC_PATH_SEGMENT,
   ...
   */
  public static boolean haveSameAccount(final URI remoteUrl1,
                                        final URI remoteUrl2) {
    if (remoteUrl1 != null && remoteUrl2 != null) {
      return StringUtils.equalsIgnoreCase(getServerAccountKey(remoteUrl1),
                                          getServerAccountKey(remoteUrl2));
    }
   ...
   * @param validator a provided method to validate the validity of the parsed
   *     result
   *                  this abstracts the authentication requirement away from
   * this utility
   * @return a parseResult,
   * parsed and validated result. If parsing isn't successful, every field is
   * null
   */
  public static ParseResult tryParse(final String gitUrl,
                                     final ParseResultValidator validator) {
    ...
    // only support http and https (ssh support will come later when the format
    // of the url is better understood)
    try {
      final String scheme =
          gitUri.getScheme() != null ? gitUri.getScheme().toLowerCase() : null;
  ...
  public static class ParseResult {
    public static final ParseResult FAILED =
        new ParseResult(null, null, null, null, null, null);
    ...
    // A ParseResult.Builder might make a lot of sense here if we really want
    // this ctor to be public.
    public ParseResult(final String serverUrl, final String collectionUrl,
                       final String collectionName, final String projectName,
                       final String repoName, final String accountName) {
    ...
    public boolean isSuccess() { return this != FAILED; }
  ...
  private static class HttpGitUrlParser {
    ...
    private static ParseResult tryParse(final URI gitUri,
                                        final ParseResultValidator validator) {
      ...
      // first assuming no "project" in git url, this is true for the
      // default/first repository in a project there is no "project" path
      // segment, collection url is all the way upto _git, and project name ==
      // repo name
      ...
      // so first attempt of no "project" segment failed, repo is not the same
      // as project, try the explicit project name at the third to last segment
      final ParseResult alternateResult =
          buildParseResult(gitUri, pathSegments, true);
    ...
    private static String[] getPathSegments(final URI gitUri) {
      ...
      final String gitUrlPath =
          gitUri.normalize().getPath().replaceAll("/$", "").replaceAll("^/",
                                                                       "");
    ...
    private static boolean isPathSegmentValid(final String[] segments) {
      ...
      // or "_git" must be the n-2 segment for a limited ref url, and n-1
      // segment is either "_optimized" or "_full"
      final String thirdToLastSegment = segments[segments.length - 3];
      if (URL_GIT_PATH_SEGMENT.equals(thirdToLastSegment) &&
          (URL_OPTIMIZED_REF_PATH_SEGMENT.equals(secondToLastSegment) ||
           URL_FULL_REF_PATH_SEGMENT.equals(secondToLastSegment))) {
        return true;
      }
    ...
    private static int getGitPathSegmentPosition(final String[] segments) {
      /* assuming the array passed in is wellformed, i.e. should have passed the
       * isPathSegmentValid test */
      final String secondToLastSegment = segments[segments.length - 2];
      if (URL_GIT_PATH_SEGMENT.equals(secondToLastSegment)) {
    ...
    buildParseResult(final URI gitUri, final String[] pathSegments,
      ...
      final StringBuilder urlBuilder = new StringBuilder(gitUri.getScheme())
                                           .append("://")
                                           .append(gitUri.getAuthority())
                                           .append("/");

      final int gitSegmentPos = getGitPathSegmentPosition(pathSegments);
      final int collectionSegmentPos =
          explicitProjectName ? gitSegmentPos - 2 : gitSegmentPos - 1;
      ...
      final String projectName = explicitProjectName
                                     ? pathSegments[collectionSegmentPos + 1]
                                     : repositoryName;

      return new ParseResult(serverUrl, collectionUrl, collectionName,
